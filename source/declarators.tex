...
the program is ill-formed if a default constructor\iref{class.ctor},
copy or move constructor, or copy or move assignment operator\iref{class.copy}
is so declared.
...
\begin{itemize}
\item be a special member function, and

\item not have default arguments.
\end{itemize}

\pnum
The type of an explicitly defaulted function
is allowed to differ from the type it would have had
...
\item
  ...
  the parameter type may be ``reference to non-const \tcode{T}'',
  where \tcode{T} is the name of the member function's class.
\end{itemize}
If the type differs in any other way, then:
\begin{itemize}
\item
  if the function is an assignment operator, and
  its return type differs or its parameter type is not a reference,
  the program is ill-formed;
\item
  otherwise, if the function is explicitly defaulted on its first declaration,
