...
the definition of an explicit specialization
outside of its namespace, or
...
If a typedef~(\ref{dcl.typedef}), a type \grammarterm{template-parameter}~(\ref{temp.arg.type}),
...
\pnum
\indextext{type!function}%
A type of either form is a \term{function type}.\footnote{As indicated by syntax, cv-qualifiers are a signficant component in function return types.}
...
or a \grammarterm{ref-qualifier}
...
\grammarterm{type-parameter}~(\ref{temp.names}).
\end{itemize}
...
\grammarterm{abstract-declarator} if the type of the parameter names
...
A default argument is implicitly converted (Clause~\ref{conv})
...
storage duration that is implicitly defined (see~\ref{basic.scope.local}).
...
\begin{ncbnf}
    attribute-specifier-seq\opt decl-specifier-seq\opt declarator \terminal{ = default ;}
\end{ncbnf}
...
\tcode{constexpr},
and may have an explicit \grammarterm{exception-specification} only if it is
compatible~(\ref{except.spec}) with the \grammarterm{exception-specification}
...
as if it had been implicitly declared~(\ref{except.spec}), and
\item in the case of a copy constructor, move constructor,
...
\begin{ncbnf}
    attribute-specifier-seq\opt decl-specifier-seq\opt declarator \terminal{ = delete ;}
\end{ncbnf}
...
\tcode{T};\footnote{As specified in~\ref{conv.ptr}, converting an integral
...
if
\tcode{T}
is a (possibly cv-qualified) class type (Clause~\ref{class}) with a user-provided
...
if
\tcode{T}
is a (possibly cv-qualified) non-union class type without a user-provided constructor,
then the object is zero-initialized and, if \tcode{T}'s implicitly-declared default
...
no \grammarterm{brace-or-equal-initializer}{s} for non-static data
...
\grammarterm{initializer-list}
as follows.\footnote{Braces cannot be elided in other uses of list-initialization.}
If the
\grammarterm{initializer-list}
...
narrow character literal, \tcode{char16_t} string literal, \tcode{char32_t} string
...
\tcode{T2}
is a class type), where \tcode{T1} is not reference-related to \tcode{T2}, and can be implicitly converted
...
\begin{itemize}
\item is an xvalue, class prvalue, array prvalue or function lvalue and
...
is not reference-related to \tcode{T2}, and can be implicitly converted to
an xvalue, class prvalue, or function lvalue of type ``\cvqual{cv3} \tcode{T3}'',
where ``\cvqual{cv1} \tcode{T1}'' is
reference-compatible with ``\cvqual{cv3} \tcode{T3}'',
...
\textit{cv2}.
If \tcode{T1} is reference-related to \tcode{T2} and the reference is an rvalue reference,
...
\begin{itemize}
\item If the initializer list has no elements and \tcode{T} is a class type with a
...
an \tcode{initializer_list} object is constructed as described below and used to
...
referenced by \tcode{T} is list-initialized, and the reference is bound to that
...
\exitexample
\item Otherwise, if
the initializer list has a single element, the object or reference
is initialized from that element;
if a narrowing conversion (see below) is required to convert the element to \tcode{T},
the program is ill-formed.
...
\pnum
An object of type \tcode{std::initializer_list<E>} is constructed from an initializer list as if the implementation allocated an array of $N$ elements of type \tcode{E}, where $N$ is the number of elements in the initializer list. Each element of that array is copy-initialized with the corresponding element of the initializer list, and the \tcode{std::initializer_list<E>} object is constructed to refer to that array. If a narrowing conversion is required to initialize any of the elements, the program is ill-formed.\enterexample
\begin{codeblock}
struct X {
...
\pnum
The lifetime of the array is the same as that of the \tcode{initializer_list} object. \enterexample
...
For \tcode{v1} and \tcode{v2}, the \tcode{initializer_list} object and array created for \tcode{\{ 1, 2, 3 \}} have full-expression lifetime. For \tcode{i3}, the \tcode{initializer_list} object and array have automatic lifetime. \exitexample \enternote The implementation is free to allocate the array in read-only memory if an explicit array with the same initializer could be so allocated. \exitnote
...
expression and the actual value after conversion will fit into the target type and will
