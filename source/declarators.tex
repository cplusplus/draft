...
\grammarterm{identifier-list} as names in the order of appearance, where \tcode{v}$_i$
...
\begin{ncbnf}
attribute-specifier-seq\opt{} decl-specifier-seq ref-qualifier\opt{} \tcode{e} brace-or-equal-initializer \terminal{;}
\end{ncbnf}

where the parts of the declaration other than the \grammarterm{declarator-id} are taken
from the corresponding decomposition declaration. The type of the \grammarterm{id-expression}
\tcode{e} is called \tcode{E}.
\begin{note}
\tcode{E} is never a reference type (Clause~\ref{expr}).
\end{note}
...
\pnum
Otherwise, if the expression \tcode{std::tuple_size<E>::value} is a
...
\tcode{e.get<i - 1>()}. Otherwise, the initializer is \tcode{get<i - 1>(e)},
where \tcode{get} is looked up in the associated namespaces~(\ref{basic.lookup.argdep}).
In either case, \tcode{get<i - 1>} is interpreted as a \grammarterm{template-id}.
\begin{note}
...
designated by \tcode{std::tuple_element<i - 1, E>::type}, each \tcode{v}$_i$
...
data members of \tcode{E}. The $i$th non-static data member of \tcode{E} in
declaration order is designated by \tcode{m}$_i$. Each \tcode{v}$_i$ is the
...
elements of the aggregate for purposes of aggregate initialization.
\begin{example}
