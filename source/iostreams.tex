...
floppy discs does not support hard links, symlinks, and many other features of
...
\rSec3[fs.def.absolute.path]{absolute path}
...
\rSec3[fs.def.canonical.path]{canonical path}

\pnum
An absolute path that has no elements that are symbolic links, and no dot or
dot-dot elements (\ref{path.generic}).

\rSec3[fs.def.directory]{directory}
...
\rSec3[fs.def.filesystem]{file system}

\pnum
A collection of files and certain of their attributes.

\rSec3[fs.def.race]{file system race}
...
\rSec3[fs.def.filename]{filename}

\pnum
The name of a file. Filenames dot and dot-dot have special meaning. The
...
\rSec3[fs.def.hardlink]{hard link}
...
\rSec3[fs.def.native.encode]{native encoding}
...
\rSec3[fs.def.pathname]{pathname}
...
\rSec3[fs.def.pathres]{pathname resolution}
...
\rSec3[fs.def.relative-path]{relative path}
...
\rSec3[fs.def.symlink]{symbolic link}
\pnum
...
\item Uses where file system  errors are routine and do not necessarily represent
...
\tcode{preferred_separator} is the backslash character (\tcode{\textbackslash}).
\exitexample

\rSec3[path.generic]{\tcode{path} generic pathname format grammar}
...
is required is permitted as an extension to distinguish between the formats.
\exitnote
...
\item the effective range \tcode{source} (\ref{path.req}),
...
its preferred-separator, \tcode{path("foo\\bar").generic_string()}
...
\item \textit{Dot}, if one or more trailing non-root \textit{slash}
...
{x{.12\hsize}x{.05\hsize}x{.68\hsize}}
\topline
\lhdr{Constant}	&
\chdr{Value}	&
\rhdr{Meaning}	\\ \capsep
...
\tcode{none} is shown in each option group for purposes of exposition;
...
{x{.21\hsize}x{.05\hsize}x{.59\hsize}}
\topline
...
{x{.20\hsize}x{.05\hsize}x{.08\hsize}x{.52\hsize}}
\topline
...
{x{.30\hsize}x{.05\hsize}x{.50\hsize}}
\topline
\lhdr{Name}	&
\chdr{Value}	&
\rhdr{Meaning}	\\ \capsep
...
\begin{itemdescr}
\pnum
\returns \tcode{status(path()\textit{[, ec]})}.
...
\begin{itemdescr}
\pnum
\returns \tcode{symlink_status(path()\tcode{[, ec]})}.
...
iterator. However, if
\tcode{(options \& directory_options::skip_permissions_denied) != directory_options::none}
...
\pnum
These functions enable use of \tcode{directory_iterator} with
...
\pnum
\postconditions \tcode{this->options() == rhs.options() \&\& this->depth() == rhs.depth() \&\&}
\tcode{this->recursion_pending() == rhs.recursion_pending()}.
\end{itemdescr}
...
\item Otherwise if \tcode{recursion_pending() \&\& is_directory(this->status()) \&\& (!is_symlink(this->symlink_status()) || (options() \& directory_options::follow_directory_symlink) != directory_options::none)}
then either directory \tcode{(*this)->path()} is recursively iterated into or,
if \tcode{(options() \& directory_options::skip_permissions_denied) != directory_options::none}
...
\rSec2[fs.op.funcs]{Operational functions}
...
\begin{itemdescr}
\pnum
\returns An absolute path composed according to the
  following table
\begin{floattable}
{\tcode{absolute_error(const path\&, const path\&)} effects}{tab:absolute.path}{l|ll}
\topline
\ohdr{ }					&
\chdr{\tcode{p.has_root_directory()}}		&
\rhdr{\tcode{!p.has_root_directory()}}		\\ \rowsep
\ohdr{\tcode{p.has_root_name()}}		&
\tcode{return p}				&
\tcode{return p.root_name()}			\\
						&
						&
...
\pnum
\enternote Canonical pathnames allow security checking of a path (e.g.
  does this path live in /home/goodguy or /home/badguy?) \exitnote
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects \tcode{copy(from, to, copy_options::none\textit{[, ec]})}.
\end{itemdescr}
...
\begin{itemize}
\item If \tcode{(options \& copy_options::create_symlinks) != copy_options::none}
      \tcode{|| (options \& copy_options::skip_symlinks) != copy_options::none}, then
      \tcode{auto f = symlink_status(from) }and if needed \tcode{auto t = symlink_status(to)}.
\item Otherwise, \tcode{auto f = status(from)} and if needed \tcode{auto t = status(to)}.
\end{itemize}

\pnum
Report an error as specified in Error reporting~(\ref{error.reporting})
  if:
\begin{itemize}
...
\begin{itemize}
\item If \tcode{(options \& copy_options::skip_symlinks) != copy_options::none}, then return.
\item Otherwise if \tcode{!exists(t) \&\& (options \& copy_options::copy_symlinks) != copy_options::none},
...
\begin{itemize}
\item If \tcode{(options \& copy_options::directories_only)<br> != copy_options::none}, then return.
\item Otherwise if \tcode{(options \& copy_options::create_symlinks)<br> != copy_options::none}, then create a symbolic link to the
      source file.
...
\pnum
Otherwise if \tcode{is_directory(f) \&\& ((options \& copy_options::recursive) != copy_options::none || options == copy_options::none)} then:
\begin{itemize}
...
\item Then, iterate over the files in \tcode{from}, as if by \tcode{for (directory_entry\& x : directory_iterator(from))}, and for each
      iteration \tcode{copy(x.path(), to/x.path().filename(), options | copy_options::\textit{unspecified})}.
\end{itemize}
...
\remarks For the signature with argument \tcode{ec}, any
  Filesystem library functions called by the implementation shall have an \tcode{error_code} argument if applicable.
...
\begin{itemdescr}
\pnum
\returns \tcode{copy_file(from, to, copy_options::none\textit{[, ec]})}.
...
\begin{itemdescr}
\pnum
\effects \tcode{\textit{function}(read_symlink(existing_symlink\textit{[, ec]}), new_symlink\textit{[, ec]})},
...
\pnum
\complexity \bigoh{n+1} where \textit{n} is the number of elements
...
\pnum
\returns \tcode{is_directory(s) ? directory_iterator(p) == directory_iterator() : file_size(p) == 0;}
...
\pnum
\returns \tcode{false} if p did not exist in the first
  place, otherwise \tcode{true}. The signature with argument \tcode{ec}
  returns \tcode{false} if an error occurs.
