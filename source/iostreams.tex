...
\tcode{c}.
Padding is determined as follows: A character sequence is produced,
initially consisting of the insertion character. If
\tcode{out.width()} is greater than one, then enough copies of
\tcode{out.fill()} are added to this sequence as necessary to pad to a
width of \tcode{out.width()} characters. If
\tcode{(out.flags() \& ios_base::adjustfield) == ios_base::left} is
\tcode{true}, the fill characters are placed after the insertion
character; otherwise, they are placed before the insertion character.
The insertion character and any required padding are inserted into
\tcode{out}; then calls \tcode{os.width(0)}.
...
\pnum
Padding is determined as described as follows: A character sequence is
produced, initially consisting of the elements defined by the
\tcode{n} characters starting at \tcode{s} widened using
\tcode{out.widen}~(\ref{basic.ios.members}). If \tcode{n} is less than
\tcode{out.width()}, then enough copies of \tcode{out.fill()} are
added to this sequence as necessary to pad to a width of
\tcode{out.width()} characters. If 
\tcode{(out.flags() \& ios_base::adjustfield) == ios_base::left} is
\tcode{true}, the fill characters are placed after the character
sequence; otherwise, they are placed before the character sequence.
