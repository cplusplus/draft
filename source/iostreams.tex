...
operating system dependent (\ref{fs.definitions}). The operating system an
...
\pnum
Behavior is undefined if calls to functions provided by this subclause introduce a file system race~(\ref{fs.definitions}).
...
and contains a pathname~(\ref{fs.definitions}).
  ...
  public:
    ...
    template <class InputIterator>
      path(InputIterator first, InputIterator last,
           const locale& loc, format fmt = auto_format);
    ~path();
...
\pnum
The filename \grammarterm{dot}~(\ref{fs.definitions}) is treated as a reference to the current directory.
The filename \grammarterm{dot-dot}~(\ref{fs.definitions}) is treated as a reference to the parent directory.
...
starting location for pathname resolution~(\ref{fs.definitions}).
...
or a pathname in the native format~(\ref{fs.definitions}).
...
by its value type:
\begin{itemize}
\item \tcode{char}: The encoding is the native narrow encoding~(\ref{fs.definitions}).
...
prone.
\end{note}
\item \tcode{wchar_t}: The encoding is the native wide encoding~(\ref{fs.definitions}).
...
wide encoding~(\ref{fs.definitions}) using the \tcode{codecvt<\brk{}wchar_t, char, mbstate_t>}
facet of \tcode{loc}.
\item
...
encoding~(\ref{fs.definitions}). The resulting wide string is then
...
\pnum
The string returned by all native format observers is in the native pathname format~(\ref{fs.definitions}).
...
\returns \tcode{true} if the pathname in the native format
  contains an absolute path (\ref{fs.definitions}), else \tcode{false}.
...
\pnum
\returns A path whose pathname in the generic format is
the normal form~(\ref{fs.definitions}) of the pathname
...
Does not resolve~(\ref{fs.definitions}) symlinks.
Does not first normalize~(\ref{fs.definitions}) \tcode{*this} or \tcode{base}.
...
\pnum
\begin{note} If normalization~(\ref{fs.definitions}) is needed
...
determines if two paths resolve~(\ref{fs.definitions}) to the same file system entity.
\begin{example}
...
\item If \tcode{value_type} is \tcode{char} and the current native
      narrow encoding~(\ref{fs.definitions}) is UTF-8,
...
\pnum
\begin{note} Because hardware failures, network failures, file system races~(\ref{fs.definitions}),
...
\returns \tcode{p} with symlinks resolved and
  the result normalized~(\ref{fs.definitions}).
...
\pnum
\postconditions The returned path is in normal form~(\ref{fs.definitions}).
