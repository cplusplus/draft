...
be equivalent to one directory separator character.
...
the last character of the generic format string is a \grammarterm{directory-separator}.
...
\pnum
For member function parameters that take character sequences representing
...
\pnum
If any of the characters in the argument or return value has
no representation in the destination encoding,
the resulting converted characters, if any, are unspecified.
...
\item \tcode{basic_string<EcharT, traits, Allocator>}. A function
  argument \tcode{const Source\&} \tcode{source} has an
...
\item A type meeting the \oldconcept{InputIterator} requirements that iterates over a NTCTS\@.
  ...
  \tcode{const Source\&} \tcode{source} has an effective range
...
\item A character array that after array-to-pointer decay results in a
  ...
  function argument \tcode{const Source\&} \tcode{source} has
  an effective range \range{source}{end} where
...
for how the value types above convert to
...
conversion to the native ordinary encoding.
\end{itemize}
...
pathname string in the native ordinary encoding. If the
native wide encoding is UTF-16 or UTF-32, and the native ordinary
...
\item Any existing \tcode{extension()}\iref{fs.path.decompose} is removed from the
    ...
    then
\item If \tcode{replacement} is not empty and does not begin with a period
    character, a period character is appended to the pathname in the generic format, then
\item
...
On Windows, the returned path's directory separator characters
...
\pnum
\begin{note}
Directory iteration does not yield directory entries for the current (dot)
...
Some operating systems do not support symbolic links at all or support
  ...
  In such cases, an error is reported as specified in \ref{fs.conform.posix}.
\end{note}
\end{itemdescr}
...
Some operating systems do not support hard links at all or support
  ...
  If the operation is unsupported, an error is reported as specified in \ref{fs.conform.posix}.
\end{note}
\end{itemdescr}
...
Same as \tcode{remove}\iref{fs.op.remove}, except that if \tcode{p} is a directory,
  first recursively deletes the contents of \tcode{p}.
\begin{note}
A symbolic link is itself removed, rather than the file it
  resolves to.
\end{note}
