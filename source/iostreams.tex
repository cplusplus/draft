...
\begin{itemdescr}
\pnum
The class \tcode{sentry} prepares an input stream
for a formatted or unformatted input operation.
\end{itemdescr}
...
\pnum
\effects
If \tcode{is.good()} is \tcode{false},
calls \tcode{is.setstate(failbit)}.
Otherwise, prepares for formatted or unformatted input as follows:
\begin{itemize}
\item
If \tcode{is.tie()} is not a null pointer and
the put area of the tied stream is not empty,
calls \tcode{is.tie()->flush()}.
If the put area of the tied stream is empty,
it is unspecified whether \tcode{flush} is called.

\item
If \tcode{noskipws} is \tcode{false} and
\tcode{is.flags() \& ios_base::skipws}
is nonzero, extracts and discards characters until
the next input character \tcode{c} is not a whitespace character.
If \tcode{is.rdbuf()->sbumpc()} or \tcode{is.rdbuf()->sgetc()} returns \tcode{traits::eof()},
calls \tcode{setstate(failbit | eofbit)} (which may throw \tcode{ios_base::failure}).
\end{itemize}
Then sets \tcode{ok} to the value of \tcode{is.good()}.

\pnum
\remarks
Determines whether the character \tcode{c} is a whitespace character by calling
\tcode{ct.is(ct.space, c)}, where \tcode{ct} is \tcode{use_facet<ctype<charT>>(is.getloc())}.

\pnum
During preparation, the constructor may call \tcode{setstate(failbit)}
(which may throw \tcode{ios_base::\brk{}failure}\iref{iostate.flags}).%
% FIXME: Where is the normative justification for this?
% FIXME: What is an "implementation-dependent operation"?
\footnote{The \tcode{sentry} constructor and destructor can also perform additional
\indextext{implementation-dependent}%
implementation-dependent operations.}

\pnum
When a formatted or unformatted input function
that is part of the standard library
constructs a \tcode{sentry} object,
and an output stream is tied to \tcode{is},
the call to \tcode{flush} may be deferred until
the input function would next call \tcode{is.rdbuf()->underflow()},
and may be omitted entirely if no such call to \tcode{underflow}
occurs within the input function.
\end{itemdescr}
...
\pnum
The class \tcode{sentry} prepares an output stream
for a formatted or unformatted output operation,
and performs cleanup steps after such an operation.
...
\pnum
\effects
If \tcode{os.good()} is \tcode{true},
prepares for formatted or unformatted output as follows:
\begin{itemize}
\item
If \tcode{os.tie()}
is not a null pointer, calls
\tcode{os.tie()->flush()}
% FIXME: This is much more vague than the corresponding wording in
% basic_istream::sentry.
unless the constructor can determine that no synchronization is necessary.
\end{itemize}
Then sets \tcode{ok} to the value of \tcode{is.good()}.
...
\begin{itemdescr}
\pnum
If \tcode{(os.flags() \& ios_base::unitbuf) \&\& !uncaught_exceptions()
\&\& os.good()}
is \tcode{true},
calls \tcode{os.rdbuf()->pubsync()}.
If that function returns -1, sets \tcode{badbit} in
