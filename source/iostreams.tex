...
\impldef{behavior of iostream classes when \tcode{traits::pos\_type} is not \tcode{streampos} or when \tcode{traits::off\_type} is not \tcode{streamoff}}.
...
\impldef{effect of calling \tcode{ios\_base::sync\_with\_stdio} after
...
\impldef{argument values to construct \tcode{basic\_ios::failure}}
argument values.%
\end{itemdescr}
...
is
\impldef{whether sequence pointers are copied by \tcode{basic\_stringbuf} move
...
\effects
\indextext{implementation-defined}%
\impldef{effect of calling \tcode{basic\_streambuf::setbuf} with non-zero arguments},
except that
\tcode{setbuf(0, 0)}
has no effect.
...
\pnum
\effects Move constructs from the rvalue \tcode{rhs}. It
is \impldef{whether sequence pointers are copied by \tcode{basic\_filebuf} move
constructor} whether the sequence pointers in \tcode{*this}
(\tcode{eback()}, \tcode{gptr()}, \tcode{egptr()},
\tcode{pbase()}, \tcode{pptr()}, \tcode{epptr()}) obtain
the values which \tcode{rhs} had. Whether they do or not, \tcode{*this}
...
Otherwise the results are \impldef{effect of calling \tcode{basic\_filebuf::setbuf} with
non-zero arguments}.
...
\tcode{basic\_filebuf::sync} when a get area exists}.
\end{itemdescr}
