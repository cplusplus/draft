...
that the character sequence represented by the given span
outlives the use of the sequence by objects of the classes in this subclause.
...
\exposid{mode} with \tcode{std::move(rhs.mode)} and
\tcode{buf} with \tcode{std::move(rhs.buf)}.
...
whether \tcode{rhs.\exposid{buf}.\brk{}empty()} returns true after the move.
...
\pnum
\constraints
\tcode{ROS} models \tcode{ranges::borrowed_range}.
\tcode{!convertible_to<ROS, std::span<charT>> \&\& convertible_to<ROS, std::span<charT const>>}
is \tcode{true}.
...
namespace std {
  ...
  public:
    ...
    // \iref{ospanstream.ctor}, constructors
    explicit basic_ospanstream(std::span<charT> s,
                               ios_base::openmode which = ios_base::out);
    basic_ospanstream(const basic_ospanstream&) = delete;
    basic_ospanstream(basic_ospanstream&& rhs);

    // \iref{ospanstream.assign}, assignment and swap
...
and \tcode{sb} with
\tcode{basic_spanbuf<charT, traits>(span, which | ios_base::out)}\iref{spanbuf.ctor}.
\end{itemdescr}
