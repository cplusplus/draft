...
\tcode{rdbuf()} shall return the null pointer value. \tcode{rhs} shall be in a valid but
unspecified state, except that \tcode{rhs.rdbuf()} shall return the
same value as it returned before the function call, and
\tcode{rhs.tie()} shall return the null pointer value.
\end{itemdescr}
...
\begin{note} For POSIX-based operating systems, the
      ...
      API \tcode{CreateDirectoryExW(existing_p.c_str(), p.c_str(), nullptr)}.
\end{note}
