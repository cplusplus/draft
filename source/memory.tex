...
is not deduced and is therefore expected to be specified explicitly by the caller.
\end{note}
...
\begin{codeblock}
D d;
unique_ptr<int, D> p1(new int, D());        // \tcode{D} is expected to be \oldconcept{MoveConstructible}
unique_ptr<int, D> p2(new int, d);          // \tcode{D} is expected to be \oldconcept{CopyConstructible}
...
\pnum
\begin{note}
\tcode{T} is not deduced and
is there therefore expected to be provided as a template argument.
\end{note}
\end{itemdescr}
...
\begin{note}
\tcode{T} is not deduced and
is therefore expected to be provided as a template argument.
\end{note}
\end{itemdescr}
