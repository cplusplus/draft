...
\pnum
\effects
Initializes \tcode{begin_} with \tcode{std::move(i)} and \tcode{end_} with
\tcode{s}.
\end{itemdescr}
...
\item Otherwise, equivalent to: \tcode{return \placeholdernc{make-unsigned-like}(end_ - begin_);}
\end{itemize}
\end{itemdescr}
...
\pnum
\effects
Initializes \tcode{current_} with \tcode{std::move(current)} and
...
template<class V>
  ...
  public:
    ...
    constexpr iterator_t<Base> operator->() const
      requires @\placeholder{has-arrow}@<iterator_t<Base>> && copyable<iterator_t<Base>>;
...
\item If \tcode{ref_is_glvalue} is \tcode{true} and
  \tcode{Base} and \tcode{range_reference_t<Base>} each model
...
\item Otherwise, if \tcode{ref_is_glvalue} is \tcode{true} and
  \tcode{Base} and \tcode{range_reference_t<Base>}
...
\pnum
\effects
Initializes \tcode{outer_} with \tcode{std::move(outer)} and
...
\effects
Initializes \tcode{parent_} with \tcode{addressof(parent)}
and \tcode{current_} with \tcode{std::move(current)}.
\end{itemdescr}
...
\pnum
\effects
Initializes \tcode{i_} with \tcode{std::move(i)}.
\end{itemdescr}
...
\tcode{iterator_traits<iterator_t<Base>>::iterator_category} models \linebreak
\tcode{derived_from<forward_iterator_tag>};
\item otherwise, \tcode{iterator_traits<iterator_t<Base>>::iterator_category}.
\end{itemize}
...
\pnum
\effects
Initializes \tcode{current_} with \tcode{std::move(current)}.
\end{itemdescr}
...
\pnum
\effects
Initializes \tcode{current_} with \tcode{std::move(i.current_)}.
\end{itemdescr}
