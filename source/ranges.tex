...
namespace std::ranges {
  ...
  namespace views { inline constexpr @\unspec@ join_with = @\unspec@; }
...
Equivalent to:
\begin{codeblock}
return ranges::min(@\exposid{parent_}@->@\exposid{remainder_}@, ranges::end(@\exposid{parent_}@->@\exposid{base_}@) - *@\exposid{parent_}@->@\exposid{current_}@);
\end{codeblock}
\end{itemdescr}
...
\begin{itemdecl}
constexpr iterator_t<V> @\exposid{find-next}@(iterator_t<V> current);                 // \expos
\end{itemdecl}
...
\begin{itemdecl}
constexpr iterator_t<V> @\exposid{find-prev}@(iterator_t<V> current)                  // \expos
  requires @\libconcept{bidirectional_range}@<V>;
\end{itemdecl}
