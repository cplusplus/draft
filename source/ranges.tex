...
\tcode{\libexposidnc{make-unsigned-like-t}<X>} denotes
...
\tcode{\libexposidnc{to-unsigned-like}(x)} is
\tcode{x} explicitly converted to
\tcode{\exposidnc{make-unsigned-like-t}<X>}.
...
if constexpr (@\exposid{is-integer-like}@<W> && @\exposid{is-integer-like}@<Bound>)
  return (@\exposid{value_}@ < 0)
    ? ((@\exposid{bound_}@ < 0)
      ? @\exposidnc{to-unsigned-like}@(-@\exposid{value_}@) - @\exposidnc{to-unsigned-like}@(-@\exposid{bound_}@)
      : @\exposidnc{to-unsigned-like}@(@\exposid{bound_}@) + @\exposidnc{to-unsigned-like}@(-@\exposid{value_}@))
    : @\exposidnc{to-unsigned-like}@(@\exposid{bound_}@) - @\exposidnc{to-unsigned-like}@(@\exposid{value_}@);
else
  return @\exposidnc{to-unsigned-like}@(@\exposid{bound_}@ - @\exposid{value_}@);
\end{codeblock}
...
namespace std::ranges {
  ...
  public:
    ...
    using value_type = W;
    using difference_type = @\exposidnc{IOTA-DIFF-T}@(W);
