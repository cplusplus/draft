...
\pnum
\returns
\tcode{\{*this, ranges::find_if(\exposid{base_}, ref(*pred_))\}}.
...
\exposid{pred_} with \tcode{s.pred_}.
\end{itemdescr}
...
\effects
Equivalent to:
\tcode{return y.\exposid{end_} == x || !invoke(*y.pred_, *x);}
\end{itemdescr}
...
\begin{itemize}
\item If \exposid{ref-is-glvalue} is \tcode{true}, \tcode{*outer_}.
\item Otherwise, \tcode{\exposid{parent_}->inner_}.
\end{itemize}
...
\pnum
\effects
Equivalent to: \tcode{return x.\exposid{i_}.\exposid{current_} == y.\exposid{i_}.\exposid{current_};}
\end{itemdescr}
...
if constexpr (@\exposconcept{tiny-range}@<Pattern>) {
  const auto & cur = x.@\exposid{i_}@.@\exposid{current_}@;
  if (cur == end) return true;
  if (pcur == pend) return x.@\exposid{incremented_}@;
  return *cur == *pcur;
} else {
  auto cur = x.@\exposid{i_}@.@\exposid{current_}@;
  if (cur == end) return true;
  if (pcur == pend) return x.@\exposid{incremented_}@;
  do {
...
\begin{itemdecl}
friend constexpr void iter_swap(const @\exposid{inner-iterator}@& x, const @\exposid{inner-iterator}@& y)
  noexcept(noexcept(ranges::iter_swap(x.@\exposid{i_}@.@\exposid{current_}@, y.@\exposid{i_}@.@\exposid{current_}@)))
  requires indirectly_swappable<iterator_t<@\exposid{Base}@>>;
\end{itemdecl}
...
\effects
Equivalent to
\tcode{ranges::iter_swap(x.\exposid{i_}.\exposid{current_}, y.\exposid{i_}.\exposid{current_})}.
\end{itemdescr}
...
cout << ranges::@\exposid{count_}@if(values_view{historical_figures}, is_even);     // prints \tcode{2}
\end{codeblock}
\end{example}
