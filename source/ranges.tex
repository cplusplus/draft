...
namespace std::ranges {
  ...
  public:
    ...
    constexpr iterator& operator--() requires @\exposconcept{decrementable}@<W>;
    constexpr iterator operator--(int) requires @\exposconcept{decrementable}@<W>;
...
\item If \tcode{T} models \libconcept{input_or_output_iterator} and
  ...
  \begin{itemize}
  \indexconcept{\idxconcept{random_access_iterator}}%
  \item \tcode{subrange\{E, E + static_cast<iter_difference_t<T>>(F)\}}
    if \tcode{T} models \cname{random_access_\-it\-er\-ator}.
  \item Otherwise,
