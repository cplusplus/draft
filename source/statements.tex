...
\nontermdef{expansion-init-list}\br
    \terminal{\{} \opt{expression-list} \terminal{\}}
\end{bnf}
...
\begin{codeblock}
{
  ...
  static constexpr auto @\exposidnc{begin}@ = @\exposidnc{begin-expr}@;     // see \ref{stmt.ranged}
  static constexpr auto @\exposidnc{end}@ = @\exposidnc{end-expr}@;         // see \ref{stmt.ranged}
...
is not a constant expression\iref{expr.const}.
\end{note}
...
causes control to pass to the end of the \grammarterm{statement}
or \grammarterm{compound-statement} of $X$.
...
results in undefined behavior\ubdef{stmt.return.flow.off}.
...
results in undefined behavior\ubdef{stmt.return.coroutine.flow.off}.
...
the variable is being initialized, the behavior is undefined\ubdef{stmt.dcl.local.static.init.recursive}.
\begin{example}
\begin{codeblock}
int foo(int i) {
...
No diagnostic is required\ifndrdef{stmt.ambig.bound.diff.parse}.
\begin{example}
\begin{codeblock}
struct T1 {
