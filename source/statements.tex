...
converted constant expression of type bool (\ref{expr.const}); this
form is called a \term{constexpr if} statement. If the value of the
converted condition is \tcode{false}, the first substatement is a
\term{discarded statement}, otherwise the second substatement, if
...
\tcode{if} statement shall be associated with a \tcode{switch} statement
(\ref{stmt.switch}) within the same \tcode{if} statement. A label
(\ref{stmt.label}) declared in a substatement of a \tcode{constexpr if}
