...
stream (described in Clause~\ref{input.output}) that can seek to any integral position within
...
operator function~(\ref{over.oper}) for any of the equality~(\ref{expr.eq}) or
...
direct-initialization~(\ref{dcl.init})
...
class member function~(\ref{class.mfct}) other than a constructor,
...
class member function~(\ref{class.mfct}) that accesses the state of an object of the class
...
stream (described in Clause~\ref{input.output}) that can seek to a position that was
