...
\pnum
Top level namespaces with a name starting with \tcode{std} and followed by a
non-empty sequence of digits are reserved for future standardization. The
behavior of a C++ program is undefined if it adds declarations or definitions
to such a namespace.

\pnum
\begin{example}
The top level namespace \tcode{std2} is reserved for use by future revisions of this standard. 
\end{example}
