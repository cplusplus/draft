...
\item it satisfies the requirements of \tcode{CopyConstructible} and
  \tcode{Destructible}~(\ref{utility.arg.requirements}),
\item the expressions shown in Table~\ref{hash}
are valid and have the indicated semantics, and
\item it satisfies all other requirements in this subclause.
\end{itemize}
...
\lhdr{Variable} &   \rhdr{Definition}   \\  \capsep
\endhead
\tcode{T, U, C}    &   any non-const, non-reference object type       \\ \rowsep
...
operationg system} \tcode{error_category} object for errors originating elsewhere.
