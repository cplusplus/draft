...
\pnum
A \grammarterm{conditional-expression} is a \term{core constant expression}
unless the evaluation of the expression following the rules of the abstract
machine would evaluate one of the following expressions:

\begin{itemize}
\item
\tcode{this}~(\ref{expr.prim.general}), except in \tcode{constexpr}
function or a \tcode{constexpr} constructor which is being evaluated as part
of the core constant expression.
...
\item
an evaluation of an expression that would exceed the implementation-defined
...
\begin{itemize}
  ...
  \item
  a non-volatile glvalue that refers to a non-volatile object
  that does not have static storage duration or thread storage duration, and
  whose lifetime began within the evalution of the core constant expression
  and has not ended;
\end{itemize}
...
\begin{itemize}
  ...
  began within the evaluation of the core constant expression and
  has not ended;
\end{itemize}
...
\item
modification of an object that has static or thread storage duration, or
whose lifetime did not begin
within the evaluation of the core constant expression or has
ended~(\ref{expr.ass}, \ref{expr.post.incr}, \ref{expr.pre.incr});
...
\pnum
A \term{constant expression} is a core constant expression that evaluates to
a glvalue that designates an object with static storage duration or a function,
or to a prvalue where:

\begin{itemize}
  \item
  each subobject of reference type evaluates to an lvalue that
  designates an object with static storage duration or a function; and

  \item
  each subobject of pointer type evaluates to the address of an
  object with static storage duration, to the address past the end
  of such an object~(\ref{expr.add}), to the address of a function,
  or to a null pointer value.
\end{itemize}
