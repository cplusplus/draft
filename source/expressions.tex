...
the applicable overload set is determined\iref{over.match.call}
and the best function or member function is chosen through
overload resolution\iref{over.match}.
For an invocation of the \tcode{return_value} member function
caused by a \keyword{co_return} statement\iref{stmt.return.coroutine}
whose operand is subject to result-initialization\iref{dcl.init},
overload resolution is first performed as if the operand were an rvalue.
In any other context, or if the first overload resolution fails,
overload resolution is performed again without altering value categories.
If overload resolution fails, the program is ill-formed.
