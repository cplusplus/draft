...
denotes a type as follows:

\begin{itemize}
\item If there are no \tcode{return} statements in the
\grammarterm{compound-statement}, or all \tcode{return} statements
return either an exprsesion of type \tcode{void} or no
\grammarterm{expression} or \grammarterm{braced-init-list}, it is as
if the \grammarterm{trailing-return-type} denotes \tcode{void};
\item otherwise, if all \tcode{return} statements return an expression
and the types of the returned expressions after lvalue-to-rvalue
conversion~(\ref{conv.lval}), array-to-pointer
conversion~(\ref{conv.array}), and function-to-pointer
conversion~(\ref{conv.func}) are the same, it is as if the
\grammarterm{trailing-return-type} denotes that common type;
...
template void f<A>();             // OK: lambda return type is \tcode{int}
\end{codeblock}
\exitexample
...
substition~(\ref{dcl.constexpr}) replaces each occurrence of
