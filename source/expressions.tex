...
% FIXME: We don't use this defined term anywhere. Remove it and
% simplify this sentence.
Define the \defnx{deduced type}{deduced type!class template deduction}
\tcode{U} of \tcode{T(x1, x2, ...)} to be \tcode{decltype(t)},
then the expression \tcode{T(x1, x2, ...)}
will construct the same value of type \tcode{U}
as the expression \tcode{U(x1, x2, ...)}.
