...
\begin{codeblock}
void f(int n) {
  [] { n = 1; };                // error: \tcode{n} is not odr-usable due to intervening \grammarterm{lambda-expression}
  struct A {
...
outside of a \deflink{discarded statement}{stmt.if}; no diagnostic required\ifndrdef{basic.def.odr.minimum.one.def}.
