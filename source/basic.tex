...
notion of type, scope, and
storage duration. Finally, this Clause presents the
...
block\iref{defns.block.stmt}. Such an object exists and retains its last-stored value during
...
In some situations the \Cpp{} runtime encounters an error that cannot be recovered,
such as when an exception is not handled, or a thread is destroyed while still running.
In such cases,
...
name\iref{basic.link}. An object has a storage
...
The \defn{locus} of a declaration\iref{basic.link} that is a declarator
...
to which any name that is the same as $N$\iref{basic.link} is bound in $S$.
