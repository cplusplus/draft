...
\pnum
For the components affected by subclause \ref{container.requirements}
that declare an \tcode{allocator_type},
...
if the following types and expressions are well-formed and
have the specified semantics.

\begin{itemize}
\item
The name \tcode{X::value_type} denotes the type \tcode{T}.
\tcode{T} is \oldconcept{Erasable} from \tcode{X}
(see~\ref{container.requirements.general}, below).
\item
The name \tcode{X::reference} denotes the type \tcode{T\&}.
\item
The name \tcode{X::const_reference} denotes the type \tcode{const T\&}.
\item
The name \tcode{X::iterator} denotes a type
that meets the forward iterator requirements\iref{forward.iterators}
with value type \tcode{T}.
The type \tcode{X::iterator} is convertible to \tcode{X::const_iterator}.
\item
The name \tcode{X::const_iterator} denotes a type
that meets the requirements of a constant iterator and
those of a forward iterator
with value type \tcode{T}.
\item
The name \tcode{X::difference_type} denotes a signed integer type,
identical to the difference type of
\tcode{X::iterator} and \tcode{X::const_iterator}.
\item
The name \tcode{X::size_type} denotes an unsigned integer type
that can represent any non-negative value of \tcode{difference_type}.
\end{itemize}

\begin{itemreq}{}
\tcode{X u;} and \tcode{X u = X();} are statements,
...
\begin{itemreq}{}
\tcode{X u(a);} and \tcode{X u = a;} are statements,
each with the following properties:
\end{itemreq}
...
\begin{itemreq}{}
\tcode{X u(rv);} and \tcode{X u = rv;} are statements,
each with the following properties:
\end{itemreq}
...
\pnum
\result
A prvalue of type \tcode{iterator};
...
\pnum
\result
A prvalue of type \tcode{const_iterator}.
...
\pnum
\result
A prvalue of type \tcode{strong_ordering}.
...
\pnum
\result
A prvalue of type \tcode{size_type}.
...
\begin{itemize}
\item
\tcode{X::allocator_type} denotes the type \tcode{A}.
...
the following statements and expressions are well-formed and
have the specified semantics,
except that for
...
The name \tcode{X::key_type} denotes the type \tcode{Key}.

\item
\indexordmem{mapped_type}%
For \tcode{map} and \tcode{multimap} only,
the name \tcode{X::mapped_type} denotes \tcode{T}.

\item
\indexordmem{value_type}%
For \tcode{set} and \tcode{multiset} only,
the name \tcode{X::value_type} denotes \tcode{Key}.

\item
For \tcode{map} and \tcode{multimap} only,
the name \tcode{X::value_type} denotes \tcode{pair<const Key, T>}.
...
The name \tcode{X::key_compare} denotes \tcode{Compare}.
\expects
\tcode{key_compare} is \oldconcept{CopyConstructible}.

\item
\indexordmem{value_compare}%
The name \tcode{X::value_compare} denotes a binary predicate type.
...
The name \tcode{X::node_type} denotes a specialization of
...
\indexlibraryctor{multimap}%
\begin{itemreq}{}
\tcode{X u = X();} and \tcode{X u;} are statements,
each with the following properties:
\end{itemreq}
...
For \tcode{unordered_set} and \tcode{unordered_multiset} only,
the name \tcode{X::value_type} denotes \tcode{Key}.
\item
For \tcode{unordered_map} and \tcode{unordered_multimap} only,
the name \tcode{X::value_type} denotes \tcode{pair<const Key, T>}.
...
The name \tcode{X::hasher} denotes the type \tcode{Hash}.
 \expects
\tcode{Hash} is a unary function object type
such that the expression \tcode{hf(k)} has type \tcode{size_t}.
\item
\indexunordmem{key_equal}%
The name \tcode{X::key_equal} denotes the type \tcode{Pred}.
 \expects
\tcode{Pred} meets the \oldconcept{CopyConstructi\-ble} requirements.
...
The name \tcode{X::local_iterator} denotes an iterator type
...
The name \tcode{X::const_local_iterator} denotes an iterator type
whose category, value type, difference type, and pointer and reference types
are the same as \tcode{X::const_iterator}'s.
A \tcode{const_local_iterator} object may be used to iterate
through a single bucket,
but may not be used to iterate across buckets.
\item
\indexunordmem{node_type}%
The name \tcode{X::node_type} denotes
...
\begin{itemreq}{}
The statements \tcode{X a = X()} and \tcode{X a;}
