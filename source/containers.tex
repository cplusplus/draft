...
\tcode{a.insert(p,t)}   &
 \tcode{iterator}       &
 \requires \tcode{T} shall be
 CopyInsertable into \tcode{X}. For \tcode{vector} and \tcode{deque},
...
\tcode{a_uniq.} \tcode{emplace(args)} &
 \tcode{pair<iterator,} \tcode{bool>} &
  ...
  \effects\ Inserts a \tcode{T} object \tcode{t} constructed with
...
\tcode{a_uniq.} \tcode{insert(t)}      &
  ...
  \requires\ If \tcode{t} is a non-const rvalue expression, \tcode{T} shall be
  \tcode{MoveInsertable} into \tcode{X}; otherwise, \tcode{T} shall be
  CopyInsertable into \tcode{X}.\br
...
\tcode{a.insert(}\br
  \tcode{i, j)}          &
  \tcode{void}                   &
  \requires\ \tcode{T} shall be \tcode{EmplaceConstructible} into \tcode{X} from \tcode{*i}.\br
...
\rSec3[unord.map.modifers]{\tcode{unordered_map} modifiers}
...
\rSec3[unord.multimap.modifers]{\tcode{unordered_multimap} modifiers}
...
namespace std {
  ...
  public:
    ...
    // construct/destroy/copy
    explicit unordered_set(size_type n = @\impdef@,
                           const hasher& hf = hasher(),
                           const key_equal& eql = key_equal(),
                           const allocator_type& a = allocator_type());
    template <class InputIterator>
...
namespace std {
  ...
  public:
    ...
    // construct/destroy/copy
    explicit unordered_multiset(size_type n = @\impdef@,
                                const hasher& hf = hasher(),
                                const key_equal& eql = key_equal(),
                                const allocator_type& a = allocator_type());
    template <class InputIterator>
