...
\tcode{f}
used at that point,
except that the scope in which a closure type is declared
(\ref{expr.prim.lambda}) — and therefore its associated namespaces —
...
\pnum
If, P has a form that contains \tcode{<i>}, and if the type of the corresponding
value of A differs from the type of \tcode{i}, deduction fails. If \tcode{P} has a form
that contains \tcode{[i]}, and if the type of \tcode{i} is not an integral type,
deduction fails.
