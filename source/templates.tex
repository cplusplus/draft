...
\nontermdef{concept-tt-parameter}\br
  template \terminal{<} template-parameter-list \terminal{>} \terminal{concept} \opt{\terminal{...}} \opt{identifier}\br
  template \terminal{<} template-parameter-list \terminal{>} \terminal{concept} \opt{identifier} \terminal{=} \opt{nested-name-specifier} template-name
\end{bnf}
...
are collectively referred to as \defnadj{template}{template parameter}{s}.
...
\item fold expanded constraints\iref{temp.constr.fold}, and
...
\pnum
To determine if \tcode{CD} is satisfied,
...
the normal form of the concept-id after substitution of \tcode{C}.
\begin{note}
...
the conjunction of the normal forms of \tcode{A<T> and C<T>}.\\
...
template<typename... T, template<typename> concept... CTs>
  int f(wrapper<T, CTs>...) requires (CTs<T> && ...);   // error: the fold expression expands mixed kind template parameters
\end{codeblock}
\end{example}
