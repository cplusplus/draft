...
class, function, or variable if and only if
\begin{itemize}
...
refer to the same type if and only if their \grammarterm{expression}{s} are
...
Expressions of the following forms are type-dependent if and only if the type
specified by the
\grammarterm{type-id},
...
if and only if it is declared with the \tcode{inline}
...
\pnum
These alternatives are considered if and only if type deduction would
otherwise fail.
