...
\grammarterm{template-parameter}.
A template parameter pack of a function template shall not be followed by another
template parameter unless that template parameter can be deduced or has a
...
// \tcode{U} cannot be deduced or specified
...
expressions. \enternote The equivalent substitution in exception specifications is
done only when the function is instantiated, at which point a program is ill-formed
if the substitution results in an invalid type or expression. \exitnote
...
\grammarterm{parameter-declaration-list}, the type of the parameter pack is a non-deduced context.
\enterexample
...
conversion function template
(call it
\tcode{P}; see~\ref{dcl.init}, \ref{over.match.conv}, and \ref{over.match.ref}
