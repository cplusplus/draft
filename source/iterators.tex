...
\term{single pass}
algorithms.
Value type T is not required to be a \tcode{CopyAssignable} type (Table~\ref{tab:copyassignable}).
...
\pnum
If \tcode{Iterator} has valid~(\ref{temp.deduct}) member
...
\begin{note} \tcode{operator->} may return a proxy. \end{note}
Each time
\tcode{operator++}
...
\rSec1[iterator.container]{Container access}
