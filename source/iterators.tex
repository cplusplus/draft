...
namespace std {
  ...
  template<class T> concept @\placeholder{dereferenceable}@     // \expos
    = requires(T& t) {
      *t;       // not required to be equality-preserving
      requires @\placeholdernc{can-reference}@<decltype(*t)>;
    };
  ...
  template<@\placeholder{dereferenceable}@ T>
    requires requires(T& t) {
      ranges::iter_move(t);
      requires @\placeholder{can-reference}@<decltype(ranges::iter_move(t))>;
    }
  ...
  // \ref{iterator.concept.weaklyincrementable}, concept \tcode{WeaklyIncrementable}
  template<class I>
    concept WeaklyIncrementable = @\seebelow@;

  // \ref{iterator.concept.incrementable}, concept \tcode{Incrementable}
  template<class I>
  ...
  // \ref{iterator.concept.bidirectional}, concept \tcode{BidirectionalIterator}
  template<class I>
  ...
  // \ref{common.alg.req}, common algorithm requirements
  // \ref{common.alg.req.indirectlymovable}, concept \tcode{IndirectlyMovable}
  ...
  // \ref{common.alg.req.indirectlycopyable}, concept \tcode{IndirectlyCopyable}
  template<class In, class Out>
  ...
  // \ref{common.alg.req.indirectlyswappable}, concept \tcode{IndirectlySwappable}
  template<class I1, class I2 = I1>
    concept IndirectlySwappable = @\seebelow@;

  // \ref{common.alg.req.indirectlycomparable}, concept \tcode{IndirectlyComparable}
  template<class I1, class I2, class R, class P1 = identity, class P2 = identity>
    concept IndirectlyComparable = @\seebelow@;

  // \ref{common.alg.req.permutable}, concept \tcode{Permutable}
  template<class I>
    concept Permutable = @\seebelow@;

  // \ref{common.alg.req.mergeable}, concept \tcode{Mergeable}
...
concept @\placeholder{cpp17-iterator}@ =
  Copyable<I> && requires(I i) {
    *i; requires @\placeholdernc{can-reference}@<decltype(*i)>;
    { ++i } -> Same<I>&;
    *i++; requires @\placeholdernc{can-reference}@<decltype(*i++)>;
  };
...
\rSec2[common.alg.req]{Common algorithm requirements}

\rSec3[common.alg.req.general]{General}
...
in addition to those that appear in the concepts' bodies\iref{range.comparisons}.
\end{note}

\rSec3[common.alg.req.indirectlymovable]{Concept \libconcept{IndirectlyMovable}}
...
\rSec3[common.alg.req.indirectlycopyable]{Concept \libconcept{IndirectlyCopyable}}
...
\rSec3[common.alg.req.indirectlyswappable]{Concept \libconcept{IndirectlySwappable}}
...
\rSec3[common.alg.req.indirectlycomparable]{Concept \libconcept{IndirectlyComparable}}
...
\rSec3[common.alg.req.permutable]{Concept \libconcept{Permutable}}
...
\rSec3[common.alg.req.mergeable]{Concept \libconcept{Mergeable}}
...
\rSec3[common.alg.req.sortable]{Concept \libconcept{Sortable}}
...
\pnum
\expects
\tcode{n} shall be negative only if
\tcode{I} models \libconcept{BidirectionalIterator}.
...
\pnum
\expects
\range{i}{bound} shall denote a range.
...
\range{last}{first} shall denote a range and
...
or model \libconcept{BidirectionalIterator}\iref{iterator.concept.bidirectional}.
