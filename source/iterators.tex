...
\begin{note}
Either the iterator type provides the \grammarterm{typedef-name}{s} directly
(in which case \tcode{iterator_traits} pick them up automatically), or
an \tcode{iterator_traits} specialization provides them.
\end{note}
...
\begin{note}
The program is ill-formed if, for an iterator type \tcode{X},
the specialization \tcode{iterator_traits<X>}\iref{iterator.traits}
is ill-formed.
\end{note}
