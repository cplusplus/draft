...
\pnum
\remarks
When a \grammarterm{brace-or-equal-initializer}
...
template <typename T, typename Cat = partial_ordering>
  concept ThreeWayComparable =
    ...
    (!ConvertibleTo<Cat, partial_ordering> || @\placeholder{partially-ordered-with}@<T, T>) &&
...
\item
  ...
  \libconcept{EqualityComparable}\iref{concept.equalitycomparable};
\item
...
template <typename T, typename U, typename Cat = partial_ordering>
  concept ThreeWayComparableWith =
    ...
    (!ConvertibleTo<Cat, partial_ordering> || @\placeholder{partially-ordered-with}@<T, U>) &&
...
\item
  ...
  \tcode{T} and \tcode{U} model
  \tcode{\libconcept{EqualityComparableWith}<T, U>}\iref{concept.equalitycomparable};
\item
  if \tcode{Cat} is convertible to \tcode{partial_ordering}:
  \begin{itemize}
  \item
...
\item
  ...
  \tcode{T} and \tcode{U} model
  \tcode{\libconcept{StrictTotallyOrderedWith}<T, U>}\iref{concept.stricttotallyordered}.
\end{itemize}
