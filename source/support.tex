...
\indexlibrary{\idxcode{bad_array_length}}%
\rSec3[bad.array.length]{Class \tcode{bad_array_length}}
...
\pnum
The class \tcode{bad_array_length} defines the type of objects thrown as
exceptions by the implementation to report an attempt to allocate an array of
runtime bound with a size less than or equal to zero or greater than an
implementation-defined limit~(\ref{dcl.array}).

\indexlibrary{\idxcode{bad_array_length}!\idxcode{bad_array_length}}%
\begin{itemdecl}
bad_array_length() noexcept;
\end{itemdecl}

\pnum
\effects constructs an object of class \tcode{bad_array_length}.
