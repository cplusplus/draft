...
\tcode{nullptr_t} object that is an lvalue can be taken. \end{note}
...
\pnum
The contents are the same as the C standard library header
\tcode{<limits.h>}. \begin{note} The types of the
...
\tcode{DBL_HAS_SUBNORM} &
\tcode{FLT_DECIMAL_DIG} &
\tcode{FLT_MIN_10_EXP} &
\tcode{LDBL_MAX} \\
...
\tcode{DBL_MAX_EXP} &
\tcode{FLT_HAS_SUBNORM} &
\tcode{FLT_TRUE_MIN} &
\tcode{LDBL_MIN_10_EXP} \\

\tcode{DBL_MIN} &
\tcode{FLT_MANT_DIG} &
\tcode{LDBL_DECIMAL_DIG} &
\tcode{LDBL_MIN_EXP} \\
...
\xref ISO C~5.2.4.2.2, 5.2.4.2.1.
