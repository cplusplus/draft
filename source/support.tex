...
not provide a handler for a thrown exception, \tcode{terminate()} shall be called~(\ref{except.terminate}).%
...
function does not provide a handler for a thrown exception, \tcode{terminate()} shall
be called.%
\indexlibrary{\idxcode{terminate}}
...
until a handler for the exception (including \tcode{unexpected()}
or \tcode{terminate()}) is activated~(\ref{except.handle}).
\enternote
...
\indexlibrary{\idxcode{returned}!\idxcode{nested_exception}}
\indexlibrary{\idxcode{nested_exception}!\idxcode{returned}}
\begin{itemdecl}
[[noreturn]] void rethrow_nested() const;
\end{itemdecl}
...
\begin{itemdescr}
\pnum
\effects If \tcode{nested_ptr()} returns a null pointer, the function calls \tcode{terminate()}.
