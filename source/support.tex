...
\ref{support.runtime}     & Other runtime support     &   \tcode{<csignal>}   \\
                          ...
                          &                           &   \tcode{<cstdarg>}   \\
                          &                           &   \tcode{<cstdlib>}   \\ \rowsep
\ref{support.coroutine}   & Coroutines support        &   \tcode{<coroutine>} \\
\end{libsumtab}
...
\pnum
Let \placeholder{P} be the promise type of
a coroutine\iref{dcl.fct.def.coroutine}. An object of type
\tcode{coroutine_handle<\placeholder{P}>} is called a \term{coroutine handle}
...
\pnum
\returns A coroutine handle \placeholder{h} referring to the coroutine.

\pnum
\ensures \tcode{addressof(\placeholder{h}.promise()) == addressof(p)}.
\end{itemdescr}
...
\pnum
Resuming a coroutine via \tcode{resume}, \tcode{operator()},
or \tcode{destroy} on an execution agent other than the one it was
suspended on has implementation-defined behavior unless each is either
an instance of \tcode{std::thread} or the thread that executes \tcode{main}.
...
by \tcode{noop_coroutine_handle}\iref{support.coroutine}.
\end{itemdescr}
...
\pnum
The header \tcode{<coroutine>} defines \tcode{suspend_never} and
\tcode{suspend_always} as follows.
\begin{codeblock}
namespace std {
  struct suspend_never {
