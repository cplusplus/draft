...
\pnum
\effects
Equivalent to\\\tcode{return reduce(first, last, typename iterator_traits<InputIterator>::value_type\{\});}.
\end{itemdescr}
...
\pnum
\complexity
O(\tcode{last - first}) applications of \tcode{binary_op}.
...
\pnum
\complexity
O(\tcode{last - first}) applications each of \tcode{unary_op} and
\tcode{binary_op}.
...
\begin{itemize}
\item
\tcode{init}, if \tcode{i} is \tcode{result}, otherwise

\item
\tcode{\textit{GENERALIZED_NONCOMMUTATIVE_SUM}(binary_op, init, *j, ...)} for every \tcode{j} in \range{first}{first + (i - result) - 1}.
\end{itemize}
...
\begin{itemize}
\item
\tcode{\textit{GENERALIZED_NONCOMMUTATIVE_SUM}(binary_op, init, *j, ...)} for every \tcode{j} in \range{first}{first + (i - result)}
if \tcode{init} is provided, or
\item
\tcode{\textit{GENERALIZED_NONCOMMUTATIVE_SUM}(binary_op, *j, ...)} for every \tcode{j} in \range{first}{first + (i - result)} otherwise.
\end{itemize}
...
\item
\tcode{\textit{GENERALIZED_NONCOMMUTATIVE_SUM}(binary_op, init, unary_op(*j), ...)} for every \tcode{j} in \range{first}{first + (i - result) - 1}.
\end{itemize}
...
for every \tcode{j} in \range{first}{first + (i - result)} otherwise.
\end{itemize}
...
may introduce data races~(\ref{res.on.data.races}). \begin{note} The random
...
and oft-questionable quality and performance. \end{note}
...
\rSec1[sf.cmath]{Additions to header \tcode{<cmath>}}%

\pnum
Table~\ref{tab:mathspecial}
summarizes the functions
that are added
to header \tcode{<cmath>}.

\begin{libsyntabadd3}
  {cmath}
  {tab:mathspecial}
\cspan{\functions} \\
\tcode{assoc_laguerre}	& \tcode{cyl_bessel_j}	& \tcode{hermite}	\\
\tcode{assoc_legendre}	& \tcode{cyl_bessel_k}	& \tcode{legendre}	\\
\tcode{beta}	        & \tcode{cyl_neumann}	& \tcode{laguerre}	\\
\tcode{comp_ellint_1}	& \tcode{ellint_1}	& \tcode{riemann_zeta}	\\
\tcode{comp_ellint_2}	& \tcode{ellint_2}	& \tcode{sph_bessel}	\\
\tcode{comp_ellint_3}	& \tcode{ellint_3}	& \tcode{sph_legendre}	\\
\tcode{cyl_bessel_i}	& \tcode{expint}	& \tcode{sph_neumann}	\\
\end{libsyntabadd3}

\pnum
Each of these functions
is provided for arguments
of types \tcode{float}, \tcode{double}, and \tcode{long double}.
The detailed signatures
added to header \tcode{<cmath>} are:

\indexlibrary{\idxhdr{cmath}}%
\begin{codeblock}
namespace std {
  // \ref{sf.cmath.Lnm}, associated Laguerre polynomials:
  double       assoc_laguerre(unsigned n, unsigned m, double x);
  float        assoc_laguerref(unsigned n, unsigned m, float x);
  long double  assoc_laguerrel(unsigned n, unsigned m, long double x);

  // \ref{sf.cmath.Plm}, associated Legendre polynomials:
  ...
  // \ref{sf.cmath.ellK}, (complete) elliptic integral of the first kind:
  double       comp_ellint_1(double k);
  float        comp_ellint_1f(float k);
  long double  comp_ellint_1l(long double k);

  // \ref{sf.cmath.ellEx}, (complete) elliptic integral of the second kind:
  double       comp_ellint_2(double k);
  float        comp_ellint_2f(float k);
  long double  comp_ellint_2l(long double k);

  // \ref{sf.cmath.ellPx}, (complete) elliptic integral of the third kind:
  double       comp_ellint_3(double k, double nu);
  float        comp_ellint_3f(float k, float nu);
  long double  comp_ellint_3l(long double k, long double nu);

  // \ref{sf.cmath.I}, regular modified cylindrical Bessel functions:
  double       cyl_bessel_i(double nu, double x);
  float        cyl_bessel_if(float nu, float x);
  long double  cyl_bessel_il(long double nu, long double x);

  // \ref{sf.cmath.J}, cylindrical Bessel functions (of the first kind):
  double       cyl_bessel_j(double nu, double x);
  float        cyl_bessel_jf(float nu, float x);
  long double  cyl_bessel_jl(long double nu, long double x);

  // \ref{sf.cmath.K}, irregular modified cylindrical Bessel functions:
  double       cyl_bessel_k(double nu, double x);
  float        cyl_bessel_kf(float nu, float x);
  long double  cyl_bessel_kl(long double nu, long double x);

  // \ref{sf.cmath.N}, cylindrical Neumann functions;
  // cylindrical Bessel functions (of the second kind):
  double       cyl_neumann(double nu, double x);
  float        cyl_neumannf(float nu, float x);
  long double  cyl_neumannl(long double nu, long double x);

  // \ref{sf.cmath.ellF}, (incomplete) elliptic integral of the first kind:
  double       ellint_1(double k, double phi);
  float        ellint_1f(float k, float phi);
  long double  ellint_1l(long double k, long double phi);

  // \ref{sf.cmath.ellE}, (incomplete) elliptic integral of the second kind:
  double       ellint_2(double k, double phi);
  float        ellint_2f(float k, float phi);
  long double  ellint_2l(long double k, long double phi);

  // \ref{sf.cmath.ellP}, (incomplete) elliptic integral of the third kind:
  double       ellint_3(double k, double nu, double phi);
  float        ellint_3f(float k, float nu, float phi);
  long double  ellint_3l(long double k, long double nu, long double phi);

  // \ref{sf.cmath.ei}, exponential integral:
  double       expint(double x);
  float        expintf(float x);
  long double  expintl(long double x);

  // \ref{sf.cmath.Hn}, Hermite polynomials:
  double       hermite(unsigned n, double x);
  float        hermitef(unsigned n, float x);
  long double  hermitel(unsigned n, long double x);

  // \ref{sf.cmath.Ln}, Laguerre polynomials:
  double       laguerre(unsigned n, double x);
  float        laguerref(unsigned n, float x);
  long double  laguerrel(unsigned n, long double x);

  // \ref{sf.cmath.Pl}, Legendre polynomials:
  double       legendre(unsigned l, double x);
  float        legendref(unsigned l, float x);
  long double  legendrel(unsigned l, long double x);

  // \ref{sf.cmath.riemannzeta}, Riemann zeta function:
  double       riemann_zeta(double x);
  float        riemann_zetaf(float x);
  long double  riemann_zetal(long double x);

  // \ref{sf.cmath.j}, spherical Bessel functions (of the first kind):
  double       sph_bessel(unsigned n, double x);
  float        sph_besself(unsigned n, float x);
  long double  sph_bessell(unsigned n, long double x);

  // \ref{sf.cmath.Ylm}, spherical associated Legendre functions:
  double       sph_legendre(unsigned l, unsigned m, double theta);
  float        sph_legendref(unsigned l, unsigned m, float theta);
  long double  sph_legendrel(unsigned l, unsigned m, long double theta);

  // \ref{sf.cmath.n}, spherical Neumann functions;
...
\pnum\index{NaN}\index{domain error}%
If any argument value
to any of the functions specified above
...
\rSec2[sf.cmath.Lnm]{Associated Laguerre polynomials}%
\index{assoc_laguerre@\tcode{assoc_laguerre}}%
\index{Laguerre polynomials $\mathsf{L}_n^m$}%
\index{L nm@$\mathsf{L}_n^m$ (associated Laguerre polynomials)}%
\begin{itemdecl}
double       assoc_laguerre(unsigned n, unsigned m, double x);
float        assoc_laguerref(unsigned n, unsigned m, float x);
long double  assoc_laguerrel(unsigned n, unsigned m, long double x);
...
\pnum\returns\
The \tcode{assoc_laguerre} functions return
\[%
  \mathsf{L}_n^m(x) =
  (-1)^m \frac{\mathsf{d} ^ m}
...
\rSec2[sf.cmath.Plm]{Associated Legendre polynomials}%
\index{assoc_legendre@\tcode{assoc_legendre}}%
\index{Legendre polynomials $\mathsf{P}_\ell^m$}%
\index{P lm@$\mathsf{P}_\ell^m$ (associated Legendre polynomials)}%
\begin{itemdecl}
double       assoc_legendre(unsigned l, unsigned m, double x);
float        assoc_legendref(unsigned l, unsigned m, float x);
long double  assoc_legendrel(unsigned l, unsigned m, long double x);
...
\pnum\returns\  The \tcode{assoc_legendre} functions return
\[%
  \mathsf{P}_\ell^m(x) =
  (1 - x^2) ^ {m/2}
  \:
...
\rSec2[sf.cmath.beta]{Beta function}%
\index{beta@\tcode{beta}}%
\index{Eulerian integral of the first kind|see{\tcode{beta}}}%
\index{beta functions $\mathsf{B}$}%
\begin{itemdecl}
double       beta(double x, double y);
float        betaf(float x, float y);
long double  betal(long double x, long double y);
...
\pnum\returns\
The \tcode{beta} functions return
\[%
  \mathsf{B}(x, y) =
...
\rSec2[sf.cmath.ellK]{(Complete) elliptic integral of the first kind}%
\index{comp_ellint_1@\tcode{comp_ellint_1}}%
\index{elliptic integrals!complete $\mathsf{K}$}%
\index{K complete@$\mathsf{K}$ (complete elliptic integrals)}%
\begin{itemdecl}
double       comp_ellint_1(double k);
float        comp_ellint_1f(float k);
long double  comp_ellint_1l(long double k);
...
\pnum\returns\
The \tcode{comp_ellint_1} functions return
\[%
  \mathsf{K}(k) =
  \mathsf{F}(k, \pi / 2),
...
\pnum See also \ref{sf.cmath.ellF}.
\end{itemdescr}

\rSec2[sf.cmath.ellEx]{(Complete) elliptic integral of the second kind}%
\index{comp_ellint_2@\tcode{comp_ellint_2}}%
\index{elliptic integrals!complete $\mathsf{E}$}%
\index{E complete@$\mathsf{E}$ (complete elliptic integrals)}%
\begin{itemdecl}
double       comp_ellint_2(double k);
float        comp_ellint_2f(float k);
long double  comp_ellint_2l(long double k);
...
\pnum\returns\
The \tcode{comp_ellint_2} functions return
\[%
  \mathsf{E}(k) =
  \mathsf{E}(k, \pi / 2),
\quad \mbox{for $|k| \le 1$}.
...
\pnum See also \ref{sf.cmath.ellE}.
\end{itemdescr}

\rSec2[sf.cmath.ellPx]{(Complete) elliptic integral of the third kind}%
\index{comp_ellint_3@\tcode{comp_ellint_3}}%
\index{elliptic integrals!complete $\mathsf{\Pi}$}%
\index{Pi complete@$\mathsf{\Pi}$ (complete elliptic integrals)}%
\begin{itemdecl}
double       comp_ellint_3(double k, double nu);
float        comp_ellint_3f(float k, float nu);
long double  comp_ellint_3l(long double k, long double nu);
...
\pnum\returns\
The \tcode{comp_ellint_3} functions return
\[%
  \mathsf{\Pi}(\nu, k) = \mathsf{\Pi}(\nu, k, \pi / 2),
		\quad \mbox{for $|k| \le 1$}.
\]

\pnum See also \ref{sf.cmath.ellP}.
\end{itemdescr}

\rSec2[sf.cmath.I]{Regular modified cylindrical Bessel functions}%
\index{cyl_bessel_i@\tcode{cyl_bessel_i}}%
\index{Bessel functions!$\mathsf{I}_\nu$}%
\index{I nu@$\mathsf{I}_\nu$ (Bessell functions)}%
\begin{itemdecl}
double       cyl_bessel_i(double nu, double x);
float        cyl_bessel_if(float nu, float x);
long double  cyl_bessel_il(long double nu, long double x);
...
\pnum\returns\
The \tcode{cyl_bessel_i} functions return
\[%
  \mathsf{I}_\nu(x) =
  \mathrm{i}^{-\nu} \mathsf{J}_\nu(\mathrm{i}x)
  =
...
\pnum See also \ref{sf.cmath.J}.
\end{itemdescr}

\rSec2[sf.cmath.J]{Cylindrical Bessel functions (of the first kind)}%
\index{cyl_bessel_j@\tcode{cyl_bessel_j}}%
\index{Bessel functions!$\mathsf{J}_\nu$}%
\index{J nu@$\mathsf{J}_\nu$ (Bessell functions)}%
\begin{itemdecl}
double       cyl_bessel_j(double nu, double x);
float        cyl_bessel_jf(float nu, float x);
long double  cyl_bessel_jl(long double nu, long double x);
...
\pnum\returns\
The \tcode{cyl_bessel_j} functions return
\[%
  \mathsf{J}_\nu(x) =
...
\pnum\remark\ The effect
of calling each of these functions
is implementation-defined
if \tcode{nu >= 128}.
\end{itemdescr}

\rSec2[sf.cmath.K]{Irregular modified cylindrical Bessel functions}%
\index{cyl_bessel_k@\tcode{cyl_bessel_k}}%
\index{Bessel functions!$\mathsf{K}_\nu$}%
\index{K nu@$\mathsf{K}_\nu$ (Bessell functions)}%
\begin{itemdecl}
double       cyl_bessel_k(double nu, double x);
float        cyl_bessel_kf(float nu, float x);
long double  cyl_bessel_kl(long double nu, long double x);
...
\pnum\returns\
The \tcode{cyl_bessel_k} functions return
\[%
  \mathsf{K}_\nu(x) =
  (\pi/2)\mathrm{i}^{\nu+1} (            \mathsf{J}_\nu(\mathrm{i}x)
			    + \mathrm{i} \mathsf{N}_\nu(\mathrm{i}x)
...
\pnum See also \ref{sf.cmath.I}.
\end{itemdescr}

\rSec2[sf.cmath.N]{Cylindrical Neumann functions}%
\index{cyl_neumann@\tcode{cyl_neumann}}%
\index{Bessel functions!$\mathsf{N}_\nu$}%
\index{Neumann functions!$\mathsf{N}_\nu$}%
\index{N nu@$\mathsf{N}_\nu$ (Neumann functions)}%
\begin{itemdecl}
double       cyl_neumann(double nu, double x);
float        cyl_neumannf(float nu, float x);
long double  cyl_neumannl(long double nu, long double x);
...
\pnum\returns\
The \tcode{cyl_neumann} functions return\[%
  \mathsf{N}_\nu(x) =
  \left\{
  \begin{array}{cl}
  \displaystyle
...
\rSec2[sf.cmath.ellF]{(Incomplete) elliptic integral of the first kind}%
\index{ellint_1@\tcode{ellint_1}}%
\index{elliptic integrals!incomplete $\mathsf{F}$}%
\index{F incomplete@$\mathsf{F}$ (incomplete elliptic integrals)}%
\begin{itemdecl}
double       ellint_1(double k, double phi);
float        ellint_1f(float k, float phi);
long double  ellint_1l(long double k, long double phi);
...
\pnum\returns\
The \tcode{ellint_1} functions return
\[%
  \mathsf{F}(k, \phi) =
...
\rSec2[sf.cmath.ellE]{(Incomplete) elliptic integral of the second kind}%
\index{ellint_2@\tcode{ellint_2}}%
\index{elliptic integrals!incomplete $\mathsf{E}$}%
\index{E incomplete@$\mathsf{E}$ (incomplete elliptic integrals)}%
\begin{itemdecl}
double       ellint_2(double k, double phi);
float        ellint_2f(float k, float phi);
long double  ellint_2l(long double k, long double phi);
...
\pnum\returns\
The \tcode{ellint_2} functions return
\[%
  \mathsf{E}(k, \phi) =
...
\rSec2[sf.cmath.ellP]{(Incomplete) elliptic integral of the third kind}%
\index{ellint_3@\tcode{ellint_3}}%
\index{elliptic integrals!incomplete $\mathsf{\Pi}$}%
\index{Pi incomplete@$\mathsf{\Pi}$ (incomplete elliptic integrals)}%
\begin{itemdecl}
double       ellint_3(double k, double nu, double phi);
float        ellint_3f(float k, float nu, float phi);
long double  ellint_3l(long double k, long double nu, long double phi);
...
\pnum\returns\
The \tcode{ellint_3} functions return
\[%
  \mathsf{\Pi}(\nu, k, \phi) =
  \int_0^\phi \! \frac{ \mathsf{d}\theta }
...
\rSec2[sf.cmath.ei]{Exponential integral}%
\index{expint@\tcode{expint}}%
\index{exponential integrals $\mathsf{Ei}$}%
\index{Ei@$\mathsf{Ei}$ (exponential integrals)}%
\begin{itemdecl}
double       expint(double x);
float        expintf(float x);
long double  expintl(long double x);
...
\pnum\returns\
The \tcode{expint} functions return
\[%
  \mathsf{Ei}(x) =
...
\rSec2[sf.cmath.Hn]{Hermite polynomials}%
\index{hermite@\tcode{hermite}}%
\index{Hermite polynomials $\mathsf{H}_n$}%
\index{H n@$\mathsf{H}_n$ (Hermite polynomials)}%
\begin{itemdecl}
double       hermite(unsigned n, double x);
float        hermitef(unsigned n, float x);
long double  hermitel(unsigned n, long double x);
...
\pnum\returns\
The \tcode{hermite} functions return
\[%
  \mathsf{H}_n(x) =
...
\rSec2[sf.cmath.Ln]{Laguerre polynomials}%
\index{laguerre@\tcode{laguerre}}%
\index{Laguerre polynomials $\mathsf{L}_n$}%
\index{L n@$\mathsf{L}_n$ (Laguerre polynomials)}%
\begin{itemdecl}
double       laguerre(unsigned n, double x);
float        laguerref(unsigned n, float x);
long double  laguerrel(unsigned n, long double x);
...
\pnum\returns\
The \tcode{laguerre} functions return
\[%
  \mathsf{L}_n(x) =
...
\rSec2[sf.cmath.Pl]{Legendre polynomials}%
\index{legendre@\tcode{legendre}}%
\index{Legendre polynomials $\mathsf{P}_\ell$}%
\index{P l@$\mathsf{P}_\ell$ (Legendre polynomials)}%
\begin{itemdecl}
double       legendre(unsigned l, double x);
float        legendref(unsigned l, float x);
long double  legendrel(unsigned l, long double x);
...
\pnum\returns\
The \tcode{legendre} functions return
\[%
  \mathsf{P}_\ell(x) =
  \frac{1}
       {2^\ell \, \ell!}
...
\rSec2[sf.cmath.riemannzeta]{Riemann zeta function}%
\index{riemann_zeta@\tcode{riemann_zeta}}%
\index{zeta functions $\zeta$}%
\begin{itemdecl}
double       riemann_zeta(double x);
float        riemann_zetaf(float x);
long double  riemann_zetal(long double x);
...
\pnum\returns\
The \tcode{riemann_zeta} functions return
\[%
  \mathsf{\zeta}(x) =
  \left\{
  \begin{array}{cl}
...
\rSec2[sf.cmath.j]{Spherical Bessel functions (of the first kind)}%
\index{sph_bessel@\tcode{sph_bessel}}%
\index{Bessel functions!$\mathsf{j}_n$}%
\index{j n@$\mathsf{j}_n$ (spherical Bessel functions)}%
\begin{itemdecl}
double       sph_bessel(unsigned n, double x);
float        sph_besself(unsigned n, float x);
long double  sph_bessell(unsigned n, long double x);
...
\pnum\returns\
The \tcode{sph_bessel} functions return
\[%
  \mathsf{j}_n(x) =
...
\rSec2[sf.cmath.Ylm]{Spherical associated Legendre functions}%
\index{sph_legendre@\tcode{sph_legendre}}%
\index{spherical harmonics $\mathsf{Y}_\ell^m$}%
\index{Legendre functions $\mathsf{Y}_\ell^m$}%
\index{Y lm@$\mathsf{Y}_\ell^m$ (spherical associated Legendre functions)}%
\begin{itemdecl}
double       sph_legendre(unsigned l, unsigned m, double theta);
float        sph_legendref(unsigned l, unsigned m, float theta);
long double  sph_legendrel(unsigned l, unsigned m, long double theta);
...
\pnum\returns\
The \tcode{sph_legendre} functions return
\[%
...
\pnum See also \ref{sf.cmath.Plm}.
\end{itemdescr}

\rSec2[sf.cmath.n]{Spherical Neumann functions}%
\index{sph_neumann@\tcode{sph_neumann}}%
\index{Bessel functions!$\mathsf{n}_n$}%
\index{Neumann functions!$\mathsf{n}_n$}%
\index{n n spherical@$\mathsf{n}_n$ (spherical Neumann functions)}%
\begin{itemdecl}
double       sph_neumann(unsigned n, double x);
float        sph_neumannf(unsigned n, float x);
long double  sph_neumannl(unsigned n, long double x);
...
\pnum\returns\
The \tcode{sph_neumann} functions return
\[%
  \mathsf{n}_n(x) =
