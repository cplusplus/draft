...
ABI tag such that \tcode{basic_vec<T, Abi1>::\brk{}size()} equals
\tcode{V::size()}.
If \tcode V is a specialization of \tcode{basic_mask}, let \tcode{Abi1}
denote an ABI tag such that \tcode{basic_mask<sizeof(T),
Abi1>::\brk{}size()} equals \tcode{V::size()}.
...
\begin{itemize}
 \item
   \tcode{\exposconcept{simd-complex}<basic_vec>} is modeled, and
 \item
...
\item \tcode{V} is an enabled specialization of \tcode{basic_vec},
...
namespace std::simd {
  ...
  public:
    ...
    template<class G> constexpr explicit basic_mask(G&& gen) noexcept;
