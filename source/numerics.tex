...
\pnum
Throughout this subclause, the \tcode{UnaryOperation}, \tcode{BinaryOperation},
\tcode{BinaryOperation1}, and \tcode{BinaryOperation2} parameters are used
whenever an algorithm expects a function object~(\ref{function.objects}).

\pnum
\begin{note}
The use of closed ranges, as well a semi-open ranges, to specify requirements
...
\requires
\begin{itemize}
\item \tcode{T} shall be \tcode{MoveConstructible} (Table~\ref{tab:moveconstructible}).
...
\pnum
\effects Equivalent to:
\tcode{return transform_reduce(first1, last1, first2, init, plus<>(), multiplies<>());}
\end{itemdescr}
...
\begin{itemize}
\item \tcode{T} shall be \tcode{MoveConstructible} (Table~\ref{tab:moveconstructible}).
\item All of \tcode{binary_op1(init, init)},
\tcode{binary_op1(init, binary_op2(*first1, *first2))},
\tcode{binary_op1(binary_op2(*first1, *first2), init)}, and
\tcode{binary_op1(binary_op2(*first1, *first2), binary_op2(*first1, *first2))}
...
\pnum
\returns \tcode{\textit{GENERALIZED_SUM}(binary_op1, init, binary_op2(*i, *(first2 + (i - first1))), ...)}
...
\begin{itemize}
\item \tcode{T} shall be \tcode{MoveConstructible} (Table~\ref{tab:moveconstructible}).
\item All of \tcode{binary_op(init, init)},
\tcode{binary_op(init, unary_op(*first))},
\tcode{binary_op(unary_op(*first), init)}, and
\tcode{binary_op(unary_op(*first), unary_op(*first))} shall be convertible to
\tcode{T}.
...
\pnum
\returns
\tcode{\textit{GENERALIZED_SUM}(binary_op, init, unary_op(*i), ...)} for every
iterator \tcode{i} in \range{first}{last}.
...
\requires
\begin{itemize}
\item If \tcode{init} is provided, \tcode{T} shall be \tcode{MoveConstructible}
(Table~\ref{tab:moveconstructible}); otherwise, \tcode{ForwardIterator1}'s value
type shall be \tcode{MoveConstructible}.
...
\begin{itemize}
\item \tcode{T} shall be \tcode{MoveConstructible} (Table~\ref{tab:moveconstructible}).
\item All of \tcode{binary_op(init, init)}, \tcode{binary_op(init, unary_op(*first))},
and \tcode{binary_op(unary_op(*first), unary_op(*first))} shall be convertible to
\tcode{T}.
...
\item If \tcode{init} is provided, all of \tcode{binary_op(init, init)},
\tcode{binary_op(init, unary_op(*first))}, and
\tcode{binary_op(unary_op(*first), unary_op(*first))} shall be convertible to
...
shall be writable~(\ref{iterator.requirements.general}) to the \tcode{result}
...
shall be \tcode{CopyConstructible} (Table~\ref{tab:copyconstructible}),
