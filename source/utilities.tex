...
\pnum
\returns \tcode{atomic_load_explicit(p, memory_order::seq_cst)}.
...
\pnum
\effects As if by \tcode{atomic_store_explicit(p, r, memory_order::seq_cst)}.
...
\pnum
\returns \tcode{atomic_exchange_explicit(p, r, memory_order::seq_cst)}.
...
\returns
\begin{codeblock}
atomic_compare_exchange_weak_explicit(p, v, w, memory_order::seq_cst,
...
\pnum
\remarks Two \tcode{shared_ptr} objects are equivalent
if they store the same pointer value and share ownership, or
if they store the same pointer value and both are empty.
...
\pnum
\tcode{Clock} shall meet the Clock requirements\iref{time.clock.req}.
...
\indextext{\idxhdr{charconv}}%
\indexlibrary{\idxhdr{charconv}}%
\begin{codeblock}
@%
