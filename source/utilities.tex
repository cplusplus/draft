...
\tcode{!is_convertible_v<const tuple<U>\&, T> \&\& !is_constructible_v<T, const tuple<U>\&> \&\& !is_same_v<T, U>}
is \tcode{true}.
\end{itemize}
The constructor is explicit if and only if
...
\tcode{sizeof...(Types) != 1}, or
(when \tcode{Types...} expands to \tcode{T} and \tcode{UTypes...} expands to \tcode{U})\linebreak
\tcode{!is_convertible_v<tuple<U>, T> \&\& !is_constructible_v<T, tuple<U>> \&\& !is_same_v<T, U>}
is \tcode{true}.
\end{itemize}
The constructor is explicit if and only if
\tcode{is_convertible<$U_i$\&\&, $T_i$>::value} is \tcode{false}
