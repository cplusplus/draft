...
\tcode{is_constructible_v<T1, allocator_arg_t, memory_resource*, Args1...>}
is \tcode{true},
then \tcode{xprime} is
\tcode{tuple_cat(make_tuple(allocator_arg, this->resource()), std::move(x))}.
\item
Otherwise, if \tcode{uses_allocator_v<T1, memory_resource*>} is \tcode{true} and
...
\tcode{is_constructible_v<T2, allocator_arg_t, memory_resource*, Args2...>}
is \tcode{true},
then \tcode{yprime} is
\tcode{tuple_cat(make_tuple(allocator_arg, this->resource()), std::move(y))}.
\item
Otherwise, if \tcode{uses_allocator_v<T2, memory_resource*>} is \tcode{true} and
...
have not been deallocated from \tcode{this}.\exitnote
\end{itemdescr}
