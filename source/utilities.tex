...
unless \tcode{is_swappable<T>::value} is \tcode{true}.
...
\tcode{is_swappable<T1>::value} is \tcode{true} and
\tcode{is_swappable<T2>::value} is \tcode{true}.
\end{itemdescr}
...
\begin{codeblock}
is_nothrow_swappable<@$T_i$@>::value
\end{codeblock}
...
\pnum
\remarks This function shall not participate in overload resolution
unless \tcode{is_swappable<$T_i$>::value} is \tcode{true}
...
namespace std {
  ...
  // \ref{optional.comp_with_t}, \tcode{T}
...
\pnum
\effects
Exchange the states of \tcode{*this} and \tcode{rhs}.
\end{itemdescr}
...
\pnum
\returns
If \tcode{*this} has a contained object of type T, \tcode{typeid(T)};
...
\pnum
\remarks This function shall not participate in overload resolution
unless \tcode{is_swappable<D>::value} is \tcode{true}.
...
If \tcode{FD} satisfies the requirements of CopyConstructible,
then the return type shall satisfy the requirements of CopyConstructible.
\enternote This implies that \tcode{FD} is MoveConstructible. \exitnote
\end{itemdescr}
...
public:
  ...
  ForwardIterator2 operator()(ForwardIterator2 first,
...
with \tcode{pat_first}, \tcode{pat_last_} with \tcode{pat_last}, and
...
\pnum
\effects
Equivalent to \tcode{return std::search(first, last, pat_first_, pat_last_, pred_);}
\end{itemdescr}
...
\pnum
\returns
The first iterator \tcode{i} in the range \range{first}{last - (pat_last_ - pat_first_)}
...
Returns \tcode{first} if \range{pat_first_}{pat_last_} is empty,
...
\pnum
\effects
Equivalent to \tcode{return boyer_moore_horspool_searcher<RandomAccessIterator, Hash, BinaryPredicate>(pat_first, pat_last, hf, pred);}
\end{itemdescr}
