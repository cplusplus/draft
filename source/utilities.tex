...
string sB = format("{:@\cxxcf\caret{}@6}", "x");         // value of \tcode{sB} is \tcode{"\cxxcf\cxxcf{}x\cxxcf\cxxcf\cxxcf"}
string sC = format("{:*@\caret{}@6}", "@\cxxcf@");         // value of \tcode{sC} is \tcode{"\cxxcf\cxxcf\cxxcf"}
\end{codeblock}
\end{example}
\begin{note}
...
The \cxxcf{} (\unicode{1f921}{clown face}) character has a field width of \tcode{2}.
...
string s2 = format("[{:?}]", "Спасибо, Виктор @\emo{♥}@!");    // \tcode{s2} has value: \tcode{["Спасибо, Виктор \emo{♥}!"]}
...
string s6 = format("[{:?}]", "@\emo{🤷🏻‍♂️}@");                 // \tcode{s6} has value: \tcode{["\emo{🤷}\textbackslash{u}\{200d\}\emo{♂}\textbackslash{u}\{fe0f\}"]}
...
string s9 = format("[{:?}]", "e\u0301\u0323");      // \tcode{s9} has value: \tcode{["ẹ́"]}
\end{codeblock}
\end{example}
