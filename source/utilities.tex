...
namespace std {
  ...
  // \ref{util.smartptr.shared.atomic}, \tcode{shared_ptr} atomic access
  template<class T>
...
\pnum
\indexlibrary{invoke@\tcode{\textit{INVOKE}}}%
Define \tcode{\textit{INVOKE}<R>(f, t1, t2, ..., tN)} as
...
if the expression
\tcode{\textit{INVOKE}<R>(declval<F\&>(), declval<ArgTypes>()...)},
...
\begin{itemdescr}
\pnum
\returns \tcode{\textit{INVOKE}<R>(f, std::forward<ArgTypes>(args)...)}~(\ref{func.require}),
...
\tcode{template <class Fn, class... ArgTypes>}\br
 ...
 The expression \tcode{\textit{INVOKE}(declval<Fn>(), declval<ArgTypes>()...)}
...
\tcode{template <class R, class Fn, class... ArgTypes>}\br
 ...
 The expression \tcode{\textit{INVOKE}<R>(declval<Fn>(), declval<ArgTypes>()...)}
 is well formed when treated as an unevaluated operand                &
