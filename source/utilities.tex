...
this indicates that \tcode{T} should be constructed in-place (as if by a call to a placement new expression) with the forwarded pack expansion as arguments for the initialization of \tcode{T}.
...
\pnum
\effects
\tcode{any(std::move(rhs)).swap(*this)}.
...
\pnum
\remarks An expression \tcode{std::addressof(E)}
...
\begin{itemdescr}
\pnum
\requires Let \tcode{\placeholder{CT}} be \tcode{common_type<unique_ptr<T1, D1>::pointer,}
...
\rSec3[memory.resource.prot]{\tcode{memory_resource} protected virtual member functions}
...
\pnum
\returns
A derived class shall implement this function to return \tcode{true} if memory allocated from this can be deallocated from other and vice-versa;
otherwise it shall return false.
\enternote
The most-derived type of other might not match the type of this.
For a derived class, D, a typical implementation of this function will compute \tcode{dynamic_cast<const D*>(\&other)} and go no further (i.e., return \tcode{false}) if it returns \tcode{nullptr}.\exitnote
\end{itemdescr}
...
\pnum
A specialization of class template \tcode{pmr::polymorphic_allocator} conforms to the \tcode{Allocator} requirements (\ref{allocator.requirements}).
Constructed with different memory resources, different instances of the same specialization of \tcode{pmr::polymorphic_allocator} can exhibit entirely different allocation behavior.
This runtime polymorphism allows objects that use \tcode{polymorphic_allocator} to behave as if they used different allocator types at run time even though they use the same static allocator type.

\indexlibrary{\idxcode{polymorphic_allocator}}%
\begin{codeblock}
template <class Tp>
class polymorphic_allocator {
  memory_resource* m_resource; // \expos
...
\pnum
\effects
Sets \tcode{m_resource} to \tcode{get_default_resource()}.
\end{itemdescr}
...
\pnum
\effects
Sets \tcode{m_resource} to \tcode{r}.
...
\pnum
\effects
Sets \tcode{m_resource} to \tcode{other.resource()}.
\end{itemdescr}
...
\pnum
\requires
\tcode{p} was allocated from a memory resource, \tcode{x}, equal to \tcode{*m_resource}, using \tcode{x.allocate(n * sizeof(Tp), alignof(Tp))}.

\pnum
\effects
Equivalent to \tcode{m_resource->deallocate(p, n * sizeof(Tp), alignof(Tp))}.
...
\enternote
Uses-allocator construction is always well formed for types that do not use allocators.\exitnote

\pnum
\effects
Construct a \tcode{T} object at \tcode{p} by uses-allocator construction
with allocator \tcode{this->resource()}
...
\pnum
\effects
Let \tcode{xprime} be a \tcode{tuple} constructed from \tcode{x} according to the appropriate rule from the following list.
\enternote
The following description can be summarized as constructing a \tcode{std::pair<T1,T2>} object at \tcode{p}
as if by separate uses-allocator construction with allocator \tcode{this->resource()}~(\ref{allocator.uses.construction})
...
Otherwise the program is ill formed.
\end{itemize}
This function constructs a \tcode{std::pair<T1,T2>} object at \tcode{p} using constructor arguments \tcode{piecewise_construct, xprime, yprime}.
\end{itemdescr}
...
\pnum
\returns
A pointer to a static-duration object of a type derived from \tcode{memory_resource} that can serve as a resource for allocating memory using \tcode{::operator new} and \tcode{::operator delete}. The same value is returned every time this function is called. For return value \tcode{p} and memory resource \tcode{r}, \tcode{p->is_equal(r)} returns \tcode{\&r == p}.
\end{itemdescr}
...
For return value \tcode{p} and memory resource \tcode{r}, \tcode{p->is_equal(r)} returns \tcode{\&r == p}.
\end{itemdescr}
...
\pnum
\effects
If \tcode{r} is non-null, sets the value of the default memory resource pointer to \tcode{r},
...
\pnum
\remarks
Calling the \tcode{set_default_resource} and \tcode{get_default_resource} functions shall not incur a data race.
A call to the \tcode{set_default_resource} function shall synchronize with subsequent calls to the \tcode{set_default_resource} and \tcode{get_default_resource} functions.
\end{itemdescr}
...
\pnum
The \tcode{synchronized_pool_resource} and \tcode{unsynchronized_pool_resource} classes (collectively, \defn{pool resource classes})
...
\item
A pool resource consists of a collection of \defn{pools}, serving requests for different block sizes.
Each individual pool manages a collection of \defn{chunks} that are in turn divided into blocks of uniform size, returned via calls to \tcode{do_allocate}.
Each call to \tcode{do_allocate(size, alignment)} is dispatched to the pool serving the smallest blocks accommodating at least \tcode{size} bytes.
\item
When a particular pool is exhausted, allocating a block from that pool results in the allocation of an additional chunk of memory from the \defn{upstream allocator} (supplied at construction), thus replenishing the pool.
With each successive replenishment, the chunk size obtained increases geometrically.
\enternote
By allocating memory in chunks, the pooling strategy increases the chance that consecutive allocations will be close together in memory.\exitnote
\item
Allocation requests that exceed the largest block size of any pool are fulfilled directly from the upstream allocator.
\item
A \tcode{pool_options} struct may be passed to the pool resource constructors to tune the largest block size and the maximum chunk size.
\end{itemize}

\pnum
A \tcode{synchronized_pool_resource} may be accessed from multiple threads without external synchronization
and may have thread-specific pools to reduce synchronization costs.
An \tcode{unsynchronized_pool_resource} class may not be accessed from multiple threads simultaneously
...
\pnum
The members of \tcode{pool_options} comprise a set of constructor options for pool resources.
...
If the value of \tcode{max_blocks_per_chunk} is zero or is greater than an implementation-defined limit, that limit is used instead.
...
If \tcode{largest_required_pool_block} is zero or is greater than an implementation-defined limit, that limit is used instead.
...
\pnum
\effects
Constructs a pool resource object that will obtain memory from \tcode{upstream} whenever the pool resource is unable to satisfy a memory request from its own internal data structures.
The resulting object will hold a copy of \tcode{upstream}, but will not own the resource to which \tcode{upstream} points.
\enternote
The intention is that calls to \tcode{upstream->allocate()} will be substantially fewer than calls to \tcode{this->allocate()} in most cases.\exitnote
...
It is unspecified if or under what conditions this constructor calls \tcode{upstream->allocate()}.
\end{itemdescr}
...
\pnum
\effects
Calls \tcode{upstream_resource()->deallocate()} as necessary to release all allocated memory.
\enternote
memory is released back to \tcode{upstream_resource()} even if \tcode{deallocate} has not been called for some of the allocated blocks.\exitnote
\end{itemdescr}
...
\pnum
\returns
The value of the \tcode{upstream} argument provided to the constructor of this object.
\end{itemdescr}
...
\pnum
\returns
A pointer to allocated storage (\ref{basic.stc.dynamic.deallocation}) with a size of at least \tcode{bytes}.
The size and alignment of the allocated memory shall meet the requirements for a class derived from \tcode{memory_resource} (\ref{memory.resource}).

\pnum
\effects
If the pool selected for a block of size \tcode{bytes} is unable to satisfy the memory request from its own internal data structures,
...
\pnum
\effects
Return the memory at \tcode{p} to the pool.
It is unspecified if or under what circumstances this operation will result in a call to \tcode{upstream_resource()->deallocate()}.
...
thus the amount of memory consumed increases monotonically until the resource is destroyed.
\item
...
it obtains additional buffers from an \defn{upstream} memory resource supplied at construction.
...
Specifically, calls to \tcode{allocate} and \tcode{deallocate} do not synchronize with one another.
\item
...
\pnum
\effects
Sets \tcode{upstream_rsrc} to \tcode{upstream} and \tcode{current_buffer} to \tcode{nullptr}.
If \tcode{initial_size} is specified, sets \tcode{next_buffer_size} to at least \tcode{initial_size};
...
\pnum
\effects
Sets \tcode{upstream_rsrc} to \tcode{upstream}, \tcode{current_buffer} to \tcode{buffer}, and \tcode{next_buffer_size} to \tcode{initial_size} (but not less than 1),
then increases \tcode{next_buffer_size} by an implementation-defined growth factor (which need not be integral).
\end{itemdescr}
...
\pnum
\effects
Calls \tcode{upstream_rsrc->deallocate()} as necessary to release all allocated memory.

\pnum
\enternote
memory is released back to \tcode{upstream_rsrc} even if some blocks that were allocated from \tcode{this} have not been deallocated from \tcode{this}.\exitnote
\end{itemdescr}
...
\pnum
\effects
If the unused space in \tcode{current_buffer} can fit a block with the specified \tcode{bytes} and \tcode{alignment},
...
where \tcode{n} is not less than \tcode{max(bytes, next_buffer_size)} and \tcode{m} is not less than \tcode{alignment},
and increase \tcode{next_buffer_size} by an implementation-defined growth factor (which need not be integral),
...
\enternote This implies that \tcode{FD} is \tcode{MoveConstructible}. \exitnote
\end{itemdescr}
...
\pnum
\effects
Returns a pair of iterators \tcode{i} and \tcode{j}
such that \tcode{i == std::search(first, last, pat_first_, pat_last_, pred_)}
and if \tcode{i == last}, then \tcode{j == last}, otherwise
\tcode{j == next(i, distance(pat_first_, pat_last_))}.
\end{itemdescr}
...
\pnum
\returns
A pair of iterators \tcode{i} and \tcode{j}
where \tcode{i} is the first iterator
in the range \range{first}{last - (pat_last_ - pat_first_)}
such that for every non-negative integer \tcode{n} less than \tcode{pat_last_ - pat_first_} the following condition holds:
\tcode{pred(*(i + n), *(pat_first_ + n)) != false},
and \tcode{j == next(i, distance(pat_first_, pat_last_))}.
...
\pnum
\returns
A pair of iterators \tcode{i} and \tcode{j} where
\tcode{i} is the first iterator \tcode{i} in the range
\range{first}{last - (pat_last_ - pat_first_)}
such that for every non-negative integer \tcode{n} less than \tcode{pat_last_ - pat_first_} the following condition holds:
\tcode{pred(*(i + n), *(pat_first_ + n)) != false},
and \tcode{j == next(i, distance(pat_first_, pat_last_))}.
Returns \tcode{make_pair(first, first)} if \range{pat_first_}{pat_last_} is empty,
...
\tcode{template <class T>}\br
  ...
  the same result as tcode{is_swappable_with<T\&, T\&>::value},
...
\enternote \tcode{sizeof...(R)} may be zero. \exitnote
