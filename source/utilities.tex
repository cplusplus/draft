...
and
\oldconcept{MoveAssignable} (\tref{moveassignable}).
...
of the three templates shall satisfy the \oldconcept{UnaryTypeTrait} requirements\iref{meta.rqmts}
...
each of the three templates shall satisfy the \oldconcept{TransformationTrait}
requirements\iref{meta.rqmts} with a member typedef \tcode{type} that denotes
the following type:
...
and shall satisfy the \oldconcept{Destructible} requirements (\tref{destructible}).
...
member typedef \tcode{type} that denotes the following type:
\begin{itemize}
...
the \oldconcept{Destructible} requirements (\tref{destructible}).
...
D>::pointer} shall
satisfy the \oldconcept{NullablePointer} requirements (\tref{nullablepointer}).
...
\tcode{remove_reference_t<D>} shall satisfy the \oldconcept{CopyAssignable}
...
\tcode{A} shall satisfy the \oldconcept{Allocator} requirements
...
satisfies the \oldconcept{Allocator} requirements (\tref{utilities.allocator.requirements}).
...
satisfy the allocator completeness requirements\iref{allocator.requirements.completeness}.
...
\pnum
\remarks The return type shall satisfy the \oldconcept{MoveConstructible} requirements. If all
of \tcode{FD} and $\tcode{TD}_i$ satisfy the \oldconcept{CopyConstructible} requirements, then the
return type shall satisfy the \oldconcept{CopyConstructible} requirements. \begin{note} This implies
...
shall satisfy the \oldconcept{CopyConst\-ruct\-ible} and \oldconcept{CopyAssignable} requirements.
...
Template parameters named \tcode{Hash} shall satisfy the \oldconcept{Hash}
...
\pnum
\requires
The value type of \tcode{RandomAccessIterator1} shall satisfy the \oldconcept{DefaultConstructible} requirements,
...
\pnum
\requires
The value type of \tcode{RandomAccessIterator1} shall satisfy the \oldconcept{DefaultConstructible},
...
\begin{itemize}
\item satisfy the \oldconcept{Hash} requirements (\tref{hash}),
with \tcode{Key} as the function
call argument type, the \oldconcept{Default\-Constructible} requirements (\tref{defaultconstructible}),
the \oldconcept{CopyAssignable} requirements (\tref{copyassignable}),
...
\tcode{template<class T>\br
 ...
 member typedef \tcode{type} denotes the type \tcode{T1};
 otherwise, \tcode{type} denotes the type \tcode{T}.\\ \rowsep

\indexlibrary{\idxcode{add_lvalue_reference}}%
\tcode{template<class T>\br
 struct add_lvalue_reference;}                     &
 If \tcode{T} is a referenceable type\iref{defns.referenceable} then
 the member typedef \tcode{type} denotes the type \tcode{T\&};
 otherwise, \tcode{type} denotes the type \tcode{T}.
 \begin{note}
...
\tcode{template<class T>}\br
 ...
 For example, when a type \tcode{T} is a type \tcode{T1\&}, the type
...
\tcode{template<class T>}\br
 ...
 \tcode{type} denotes the type \tcode{T}; otherwise,
...
\tcode{template<class T>\br
 ...
 the member typedef \tcode{type} is \tcode{U},
 otherwise \tcode{T}.
...
\tcode{template<class T>\br
 ...
 denotes the type \tcode{T1};
 otherwise, it denotes the type \tcode{T}.\\ \rowsep
...
\tcode{template<class T>\br
 ...
 the member typedef \tcode{type} denotes the same type as
...
\tcode{template<class T>\br
 ...
 &
 The member typedef \tcode{type} denotes the type \tcode{T}. \\ \rowsep
...
\tcode{template<size_t Len,\br
 ...
 struct aligned_storage;}
 &
 The value of \textit{default-alignment} is the most
...
\tcode{template<size_t Len,\br
  ...
  its size is at least \tcode{Len}. The static member \tcode{alignment_value}
  is an integral constant of type \tcode{size_t} whose value is the
...
\tcode{template<class T>\br struct decay;}
 ...
 \tcode{true}, the member typedef \tcode{type} denotes the type
 \tcode{remove_extent_t<U>*}. If \tcode{is_function_v<U>} is \tcode{true},
 the member typedef \tcode{type} denotes the type \tcode{add_pointer_t<U>}.
 Otherwise the member typedef \tcode{type} denotes the type \tcode{remove_cv_t<U>}.
...
\tcode{template<bool B, class T = void>} \tcode{struct enable_if;}
 ...
 denotes the type \tcode{T}; otherwise, there is no member
 \tcode{type}. \\ \rowsep
...
\tcode{template<bool B, class T,}
 ...
 &
 If \tcode{B} is \tcode{true},  the member typedef \tcode{type} denotes the type \tcode{T}.
 If \tcode{B} is \tcode{false}, the member typedef \tcode{type} denotes the type \tcode{F}. \\ \rowsep
