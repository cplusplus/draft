...
\pnum
For the \tcode{to_chars} function taking a parameter \tcode{base},
...
For the \tcode{from_chars} function taking a parameter \tcode{base},
the implementation shall provide overloads
for all signed and unsigned integer types
and \tcode{char}
...
\begin{itemdecl}
to_chars_result to_chars(char* first, char* last, @\UNSP{see above}@ value, int base = 10);
\end{itemdecl}
...
\begin{itemdecl}
from_chars_result from_chars(const char* first, const char* last,
                             @\UNSP{see above}@& value, int base = 10);
\end{itemdecl}
...
as described for \tcode{strtol} in ISO C~7.22.1.4,
...
\effects The pattern is the expected form of the subject sequence
in the \tcode{"C"} locale,
as described for \tcode{strtod} in ISO C~7.22.1.3,
except that
\begin{itemize}
\item
