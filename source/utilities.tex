...
namespace std {
  ...
  // \ref{variant.in.place}, in-place construction
...
\rSec2[variant.variant]{\tcode{variant} of value types}
...
\remarks
This function shall not participate in overload resolution unless there is
exactly one occurrences of \tcode{T} in \tcode{Types...} and
\tcode{is_constructible_v<T, initializer_list<U>\&, Args...>} is \tcode{true}.
If \tcode{T}'s selected constructor is a constexpr constructor, this
constructor shall be a constexpr constructor.
\end{itemdescr}
...
\item
if \tcode{index() == rhs.index()}, calls \tcode{swap(get<i>(*this), get<i>(rhs))} where \tcode{i} is \tcode{index()}. Otherwise,
\item
...
\begin{itemdescr}
\pnum
Let \tcode{VS} denote \tcode{variant_alternative<T>} of the
...
\rSec2[variant.in.place]{In-place construction}

\indexlibrary{\idxcode{in_place_type_t}}%
\indexlibrary{\idxcode{in_place_type}}%
\indexlibrary{\idxcode{in_place_index_t}}%
\indexlibrary{\idxcode{in_place_index}}%
\begin{itemdecl}
template <class T> struct in_place_type_t{ explicit in_place_type_t() = default; };
template <class T> constexpr in_place_type_t<T> in_place_type{};
template <size_t I> struct in_place_index_t{ explicit in_place_index_t() = default; };
...
\begin{itemdescr}
\pnum
Template specializations of \tcode{in_place_type_t} are empty structure types
used as unique types to disambiguate constructor overloading. They signal
(through the template parameter) the alternative to be constructed.
Specifically, \tcode{variant} has a constructor with \tcode{in_place_type_t<T>}
as the first argument followed by an argument pack; this indicates that
\tcode{T} should be constructed in-place (as if by a call to a placement
\grammarterm{new-expression}) with the forwarded argument pack as parameters.
If a \tcode{variant}'s \tcode{Types...} has multiple occurrences of \tcode{T},
\tcode{in_place_index_t} shall be used.

\pnum
Template specializations of \tcode{in_place_index_t} are empty structure types
used as unique types to disambiguate constructor overloading, and signaling
(through the template parameter) the alternative to be constructed.
Specifically, \tcode{variant} has a constructor with \tcode{in_place_index_t<I>}
as the first argument followed by an argument pack; this indicates that
$T_I$ should be constructed in-place (as if by a call to a placement
\grammarterm{new-expression}) with the forwarded argument pack as parameters.
\end{itemdescr}
...
return (v.index() < w.index()) ||
       (v.index() == w.index() && !v.valueless_by_exception() && get<@$i$@>(v) < get<@$i$@>(w))
\end{codeblock}
with $i$ being \tcode{v.index()}.
\end{itemdescr}
...
The contents of the header \tcode{<ctime>} are the same as the Standard C library header \tcode{<time.h>}.%
