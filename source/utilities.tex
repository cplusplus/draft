...
Equivalent to:
\begin{codeblock}
return @\placeholder{make-from-tuple-impl}@<T>(
           forward<Tuple>(t),
           make_index_sequence<tuple_size_v<remove_reference_t<Tuple>>>{});
\end{codeblock}
...
namespace std {
  ...
  namespace ranges {
    template<@\placeholdernc{no-throw-forward-iterator}@ I, @\placeholder{no-throw-sentinel}@<I> S>
  ...
  namespace ranges {
    ...
    template<input_iterator I, sentinel_for<I> S1,
             @\placeholdernc{no-throw-forward-iterator}@ O, @\placeholder{no-throw-sentinel}@<O> S2>
    ...
    template<input_iterator I, @\placeholdernc{no-throw-forward-iterator}@ O, @\placeholder{no-throw-sentinel}@<O> S>
      requires constructible_from<iter_value_t<O>, iter_reference_t<I>>
  ...
  namespace ranges {
    ...
    template<input_iterator I,
             @\placeholdernc{no-throw-forward-iterator}@ O, @\placeholder{no-throw-sentinel}@<O> S>
      requires constructible_from<iter_value_t<O>, iter_rvalue_reference_t<I>>
  ...
  namespace ranges {
    template<@\placeholdernc{no-throw-forward-iterator}@ I, @\placeholder{no-throw-sentinel}@<I> S, class T>
  ...
  namespace ranges {
    ...
    template<@\placeholdernc{no-throw-input-iterator}@ I, @\placeholder{no-throw-sentinel}@<I> S>
...
concept @\placeholdernc{no-throw-input-range}@ = // \expos
  ...
  @\placeholder{no-throw-sentinel}@<sentinel_t<R>, iterator_t<R>>;
\end{itemdecl}
...
\pnum
\begin{note}
Defining a comparison function allows \tcode{shared_ptr} objects
...
\pnum
\indexlibrary{invoke@\tcode{\placeholder{INVOKE}}}%
Define \tcode{\placeholder{INVOKE}<R>(f, t$_1$, t$_2$, $\dotsc$, t$_N$)} as
...
\begin{codeblock}
@\placeholder{INVOKE}@<R>(static_cast<@$\tcode{V}_\tcode{fd}$@>(@$\tcode{v}_\tcode{fd}$@),
          static_cast<@$\tcode{V}_1$@>(@$\tcode{v}_1$@), static_cast<@$\tcode{V}_2$@>(@$\tcode{v}_2$@), @$\dotsc$@, static_cast<@$\tcode{V}_N$@>(@$\tcode{v}_N$@))
\end{codeblock}
for the second overload,
...
if the expression
\tcode{\placeholder{INVOKE}<R>(declval<F\&>(), declval<ArgTypes>()...)},
...
\begin{itemdescr}
\pnum
\returns \tcode{\placeholder{INVOKE}<R>(f, std::forward<ArgTypes>(args)...)}\iref{func.require},
...
\indexlibrary{\idxcode{has_strong_structural_equality}}%
\tcode{template<class T>}\br
...
\tcode{template<class R, class Fn, class... ArgTypes>}\br
 ...
 The expression \tcode{\placeholder{INVOKE}<R>(declval<Fn>(), declval<ArgTypes>()...)}
 is well-formed when treated as an unevaluated operand                &
