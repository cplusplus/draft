...
\pnum
Implementations are encouraged to avoid the use of dynamically allocated memory
...
\tcode{is_nothrow_invocable_r_v<R, VT \cv{} \placeholder{ref}, Args...> \&\&
is_nothrow_invocable_r_v<R, VT \placeholder{inv-quals}, Args...>}.
Otherwise, \tcode{\exposid{is-callable-\linebreak from}<VT>} is equal to
\tcode{is_invocable_r_v<R, VT \cv{} \placeholder{ref}, Args...> \&\&
is_invocable_r_v<R, VT \placeholder{inv-quals}, Args...>}.
\end{itemdescr}
...
\pnum
\ensures
\tcode{*this} has a target object unless any of the following hold:
\begin{itemize}
\item
...
\end{itemize}
Otherwise, \tcode{*this} targets an object of type \tcode{VT}
direct-non-list-initialized with \tcode{std::forward<F>(f)}.
...
\pnum
\ensures
\tcode{*this} targets an object of type \tcode{VT}
direct-non-list-initialized with\newline \tcode{std::forward<Args>(args)...}.
...
\tcode{*this} targets an object of type \tcode{VT}
direct-non-list-initialized with\newline
\tcode{ilist, std::forward<ArgTypes>(args)...}.
...
\end{codeblock}
where \tcode{f} is the target object of \tcode{*this} and
\tcode{f} is lvalue of type \tcode{F}.
\end{itemdescr}
...
\pnum
\effects
Exchanges the targets of \tcode{*this} and \tcode{other}.
\end{itemdescr}
...
\pnum
\returns
\tcode{true} if \tcode{f} has no target object.
\end{itemdescr}
\indextext{function object!wrapper|)}
