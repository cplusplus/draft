...
\begin{itemdescr}
\pnum
\requires is_assignable<first_type\&, const U\&>::value is \tcode{true}
and is_assignable<second_type\&, const V\&>::value is \tcode{true}.
...
namespace std {
  ...
  public:
    ...
    template <class U1, class U2>
      tuple& operator=(pair<U1, U2>&&) noexcept;         // iff \tcode{sizeof...(Types) == 2}
...
\pnum
The header \tcode{<memory>} defines several types and template functions that
...
\tcode{r} is unspecified; otherwise, it is \tcode{T\&}.

\returns The first template function returns a dereferenceable pointer to \tcode{r}
...
function. The second template function returns \tcode{std::addressof(r)}.
\end{itemdescr}
...
\pnum
\remarks This constructor shall not participate in overload resolution unless
\tcode{U*} is implictly convertible to \tcode{T*} and \tcode{D} is
...
of the deleter from an rvalue of type \tcode{D} shall not throw an execption.
...
\pnum
\returns
The first template function returns \tcode{nullptr < x}.
The second template function returns \tcode{x < nullptr}.
\end{itemdescr}
...
\pnum
\returns
The first template function returns \tcode{!(x < nullptr)}.
The second template function returns \tcode{!(nullptr < x)}.
\end{itemdescr}
...
\pnum
\notes Implementations are encouraged, but not required, to
...
\pnum
\returns
The first template function returns \tcode{nullptr < a}.
The second template function returns \tcode{a < nullptr}.
\end{itemdescr}
...
\pnum
\returns
The first template function returns \tcode{!(a < nullptr)}.
The second template function returns \tcode{!(nullptr < a)}.
\end{itemdescr}
...
uses \tcode{is_bind_expression} to detect subexpressions. Users may
specialize this template to indicate that a type should be treated
as a subexpression in a \tcode{bind} call.

\pnum
If \tcode{T} is a type returned from \tcode{bind}, \tcode{is_bind_expression<T>}
shall be publicly derived from \tcode{integral_constant<bool, true>}, otherwise
from \tcode{integral_constant<bool, false>}.

\pnum
\tcode{is_placeholder} can be used to detect the standard placeholders
\tcode{_1}, \tcode{_2}, and so on. \tcode{bind}
uses \tcode{is_placeholder} to detect placeholders. Users may
specialize this template to indicate a placeholder type.

\pnum
If \tcode{T} is the type of \tcode{std::placeholders::_J}, \tcode{is_placeholder<T>}
shall be publicly derived from \tcode{integral_constant<int, J>}, otherwise from
\tcode{integral_constant<int, 0>}.
...
\item \tcode{ti} is the $i^{th}$ argument in the function paramater pack \tcode{bound_args},
...
be \tcode{\textit{INVOKE}(fd, v1, v2, ..., vN, result_of<FD \textit{cv}
...
\tcode{g(u1, u2, ..., uM)} shall be \tcode{\textit{INVOKE}(fd, v1, v2,
...
type \tcode{Vi} is \tcode{result_of<TiD \textit{cv} (Uj...)>::type};
...
template<class R, class T>
  @\unspec@ mem_fn(R T::* pm);
template<class R, class T, class... Args>
  @\unspec@ mem_fn(R (T::* pm)(Args...));
template<class R, class T, class... Args>
  @\unspec@ mem_fn(R (T::* pm)(Args...) const);
template<class R, class T, class... Args>
  @\unspec@ mem_fn(R (T::* pm)(Args...) volatile);
template<class R, class T, class... Args>
  @\unspec@ mem_fn(R (T::* pm)(Args...) const volatile);
template<class R, class T, class... Args>
  @\unspec@ mem_fn(R (T::* pm)(Args...) &);
template<class R, class T, class... Args>
  @\unspec@ mem_fn(R (T::* pm)(Args...) const &);
...
namespace std {
  ...
  public:
    ...
    typedef T1 argument_type;           // iff \tcode{sizeof...(ArgTypes) == 1} and
                                        // the type in \tcode{ArgTypes} is \tcode{T1}
    typedef T1 first_argument_type;     // iff \tcode{sizeof...(ArgTypes) == 2} and
                                        // \tcode{ArgTypes} contains \tcode{T1} and \tcode{T2}
    typedef T2 second_argument_type;    // iff \tcode{sizeof...(ArgTypes) == 2} and
                                        // \tcode{ArgTypes} contains \tcode{T1} and \tcode{T2}
...
specialisations of any of these templates.
...
\tcode{template <class T>}\br
  ...
  For a complete type \tcode{T} and given\br
  \tcode{template <class U>}
  \tcode{struct test \{ U u; \};},\br
  \tcode{test<T>::\~{}test()} is not deleted. &
  \tcode{T} shall be a complete type, (possibly \cv-qualified) \tcode{void},
  or an array of unknown bound. \\ \rowsep
...
\tcode{template <class Base, class Derived>}\br
 ...
 \tcode{Base} is a base class of \tcode{Derived}~(\ref{class.derived})
 without regard to cv-qualifiers
 or \tcode{Base} and \tcode{Derived} are not unions and
 name the same class type
 without regard to cv-qualifiers&
 If \tcode{Base} and
 \tcode{Derived} are class types and are different types (ignoring
...
\tcode{template <std::size_t Len,}\br
  ...
  shall be an integral cosntant of type \tcode{std::size_t} whose value is the
...
\tcode{template <bool B, class T = void>} \tcode{struct enable_if;} & &
 ...
 shall equal \tcode{T}; otherwise, there shall be no member typedef
 \tcode{type}. \\ \rowsep
...
\tcode{template <class Fn,}\br
 ...
 \tcode{Fn} shall be a callable
 type~(\ref{func.def}),
 reference to function, or reference to callable type. The
 expression
...
\begin{itemdescr}
\pnum
\returns \tcode{CT(lhs) += rhs}, where \tcode{CT} is the type of the return value.
\end{itemdescr}
...
public:
  ...
  static const bool is_steady =            @\unspec;@
...
\indexlibrary{\idxcode{<scoped_allocator>}}%
\begin{codeblock}
...
\begin{itemdescr}
\pnum
\returns \tcode{static_cast<Outer\&>(*this)}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\returns \tcode{static_cast<const Outer\&>(*this)}.
\end{itemdescr}
