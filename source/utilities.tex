...
\defn{optional objects}. An \defn{optional object for object types} is an
object that contains the storage for another object and manages the lifetime
of this contained object, if any. The contained object may be initialized after
...
namespace std {
  ...
  // \ref{optional.nullopt}, Disengaged state indicator
  struct nullopt_t{@\seebelow@};
  constexpr nullopt_t nullopt(@\unspec@);

  // \ref{optional.bad_optional_access}, class bad_optional_access
  ...
  // \ref{optional.nullops}, Comparison with nullopt
  template <class T> constexpr bool operator==(const optional<T>&, nullopt_t) noexcept;
  template <class T> constexpr bool operator==(nullopt_t, const optional<T>&) noexcept;
  template <class T> constexpr bool operator<(const optional<T>&, nullopt_t) noexcept;
  template <class T> constexpr bool operator<(nullopt_t, const optional<T>&) noexcept;

  // \ref{optional.comp_with_t}, Comparison with T
...
\rSec2[optional.defs]{Definitions}

\pnum
An instance of \tcode{optional<T>} is said to be \defn{disengaged} if
\begin{itemize}
\item it is default-initialized; or
\item it is initialized with a value of type \tcode{nullopt_t} or with a
disengaged optional object of type \tcode{optional<T>}; or
\item a value of type \tcode{nullopt_t} or a disengaged optional object of
type \tcode{optional<T>} is assigned to it.
\end{itemize}

\pnum
An instance of \tcode{optional<T>} is said to be \defn{engaged} if it is not
disengaged.
...
\pnum
An engaged instance of \tcode{optional<T>} where \tcode{T} is of object type
shall contain a value of type \tcode{T} within its own storage. This value is
referred to as the \defn{contained value} of the optional object.
Implementations are not permitted to use additional storage, such as dynamic
memory, to allocate its contained value. The contained value shall be allocated
in a region of the \tcode{optional<T>} storage suitably aligned for the type
\tcode{T}.

\pnum
Members \tcode{init} and \tcode{val} are provided for exposition only.
Implementations need not provide those members. \tcode{init} indicates whether
the optional object's contained value has been initialized (and not yet
destroyed); when \tcode{init} is true \tcode{val} points to the contained value.

\pnum
\tcode{T} shall be an object type and shall satisfy the requirements of
\tcode{Destructible} (Table~\ref{tab:destructible}).
...
\begin{itemdescr}
\pnum
\postconditions \tcode{*this} is disengaged.

\pnum
\remarks No contained value is initialized. For every object type
\tcode{T} these constructors shall be \tcode{constexpr} constructors~(\ref{dcl.constexpr}).
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\requires \tcode{is_copy_constructible<T>::value} is true.

\pnum
\effects If \tcode{rhs} is engaged initializes the contained value as if
direct-non-list-initializing an object of type \tcode{T} with the expression
\tcode{*rhs}.

\pnum
\postconditions \tcode{bool(rhs) == bool(*this)}.
...
\indexlibrary{\idxcode{optional}!constructor}%
\begin{itemdecl}
optional<T>::optional(optional<T>&& rhs) noexcept(@\seebelow@);
\end{itemdecl}

\begin{itemdescr}
\pnum
\requires \tcode{is_move_constructible<T>::value} is true.

\pnum
\effects If \tcode{rhs} is engaged initializes the contained value as if
direct-non-list-initializing an object of type \tcode{T} with the expression
\tcode{std::move(*rhs)}. \tcode{bool(rhs)} is unchanged.
...
object of type \tcode{T} with the expression \tcode{v}.

\pnum
\postconditions \tcode{*this} is engaged.

\pnum
\throws Any exception thrown by the selected constructor of \tcode{T}.

\pnum
\remarks If \tcode{T}'s selected constructor is a \tcode{constexpr}
constructor, this constructor shall be a \tcode{constexpr} constructor.
\end{itemdescr}
...
\pnum
\effects Initializes the contained value as if direct-non-list-initializing an
object of type \tcode{T} with the expression \tcode{std::move(v)}.
...
\begin{itemdescr}
\pnum
\requires \tcode{is_constructible<T, Args\&\&...>::value} is true.

\pnum
\effects Initializes the contained value as if constructing an object of type
\tcode{T} with the arguments \tcode{std::forward<Args>(args)...}.
...
\pnum
\remarks If \tcode{T}'s constructor selected for the initialization is a
\tcode{constexpr} constructor, this constructor shall be a \tcode{constexpr}
constructor.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\requires \tcode{is_constructible<T, initializer_list<U>\&, Args\&\&...>::value}
is true.

\pnum
\effects Initializes the contained value as if constructing an object of type
\tcode{T} with the arguments \tcode{il, std::forward<Args>(args)...}.
...
\tcode{is_constructible<T, initializer_list<U>\&, Args\&\&...>::value} is true.
...
\begin{itemdescr}
\pnum
\effects If \tcode{is_trivially_destructible<T>::value != true} and
\tcode{*this} is engaged, calls \tcode{val->T::\~{}T()}.

\pnum
\remarks If \tcode{is_trivially_destructible<T>::value == true} then this
...
\begin{itemdescr}
\pnum
\effects If \tcode{*this} is engaged calls \tcode{val->T::\~{}T()} to destroy
...
\begin{itemdescr}
\pnum
\requires \tcode{is_copy_constructible<T>::value} is true and
\tcode{is_copy_assignable<T>::value} is true.

\pnum
\effects
\begin{itemize}
\item If \tcode{*this} is disengaged and \tcode{rhs} is disengaged, no effect, otherwise
\item if \tcode{*this} is engaged and \tcode{rhs} is disengaged, destroys
the contained value by calling \tcode{val->T::\~{}T()}, otherwise
\item if \tcode{*this} is disengaged and \tcode{rhs} is engaged, initializes
the contained value as if direct-non-list-initializing an object of type
\tcode{T} with \tcode{*rhs}, otherwise
\item (if both \tcode{*this} and \tcode{rhs} are engaged) assigns \tcode{*rhs}
...
\pnum
\exceptionsafety If any exception is thrown, the values of \tcode{init} and
\tcode{rhs.init} remain unchanged. If an exception is thrown during the call to
\tcode{T}'s copy constructor, no effect. If an exception is thrown during the
call to \tcode{T}'s copy assignment, the state of its contained value is as
defined by the exception safety guarantee of \tcode{T}'s copy constructor.
\end{itemdescr}

\indexlibrary{\idxcode{optional}!\idxcode{operator=}}%
\indexlibrary{\idxcode{operator=}!\idxcode{optional}}%
\begin{itemdecl}
optional<T>& optional<T>::operator=(optional<T>&& rhs) noexcept(@\seebelow@);
\end{itemdecl}

\begin{itemdescr}
\pnum
\requires \tcode{is_move_constructible<T>::value} is true and
\tcode{is_move_assignable<T>::value} is true.
...
\begin{itemize}
\item If \tcode{*this} is disengaged and \tcode{rhs} is disengaged, no effect, otherwise
\item if \tcode{*this} is engaged and \tcode{rhs} is disengaged, destroys the
contained value by calling \tcode{val->T::\~{}T()}, otherwise
\item if \tcode{*this} is disengaged and \tcode{rhs} is engaged, initializes
the contained value as if direct-non-list-initializing an object of type
\tcode{T} with \tcode{std::move(*rhs)}, otherwise
\item (if both \tcode{*this} and \tcode{rhs} are engaged) assigns
\tcode{std::move(*rhs)} to the contained value.
\end{itemize}
...
\tcode{is_nothrow_move_assignable<T>::value \&\& is_nothrow_move_constructible<T>::value}

\pnum
\exceptionsafety If any exception is thrown, the values of \tcode{init} and
\tcode{rhs.init} remain unchanged. If an exception is thrown during the call to
\tcode{T}'s move constructor, the state of \tcode{*rhs.val} is determined by
the exception safety guarantee of \tcode{T}'s move constructor. If an
exception is thrown during the call to \tcode{T}'s move assignment, the state
of \tcode{*val} and \tcode{*rhs.val} is determined by the exception safety
guarantee of \tcode{T}'s move assignment.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\requires \tcode{is_constructible<T, U>::value} is true and
\tcode{is_assignable<U, T>::value} is true.

\pnum
\effects If \tcode{*this} is engaged assigns \tcode{std::forward<U>(v)} to the
contained value; otherwise initializes the contained value as if
direct-non-list-initializing an object of type \tcode{T} with
\tcode{std::forward<U>(v)}.
...
\pnum
\exceptionsafety If any exception is thrown, the value of \tcode{init} remains
unchanged. If an exception is thrown during the call to \tcode{T}'s constructor,
the state of \tcode{v} is determined by the exception safety guarantee of
\tcode{T}'s constructor. If an exception is thrown during the call to
\tcode{T}'s assignment, the state of \tcode{*val} and \tcode{v} is determined
by the exception safety guarantee of \tcode{T}'s assignment.

\pnum
\remarks This function shall not participate in overload resolution unless
\tcode{is_same<remove_reference\brk{}<U>::type, T>::value} is true.

\pnum
\enternote The reason for providing such generic assignment and then
constraining it so that effectively \tcode{T == U} is to guarantee that
assignment of the form \tcode{o = \{\}} is unambiguous. \exitnote
\end{itemdescr}
...
\pnum
\effects Calls\tcode{ *this = nullopt}. Then initializes the contained value as
if constructing an object of type \tcode{T} with the arguments
...
\pnum
\exceptionsafety If an exception is thrown during the call to \tcode{T}'s
constructor, \tcode{*this} is disengaged, and the previous \tcode{*val} (if any)
has been destroyed.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\requires Lvalues of type \tcode{T} shall be swappable and
\tcode{is_move_constructible<T>::value} is true.

\pnum
\effects
\begin{itemize}
\item If \tcode{*this} is disengaged and \tcode{rhs} is disengaged, no effect, otherwise
\item if \tcode{*this} is engaged and \tcode{rhs} is disengaged, initializes
the contained value of \tcode{rhs} by direct-initialization with
\tcode{std::move(*(*this))}, followed by \tcode{val->T::\~{}T()},
\tcode{swap(init, rhs.init)}, otherwise
\item if \tcode{*this} is disengaged and \tcode{rhs} is engaged, initializes
the contained value of \tcode{*this} by direct-initialization with
\tcode{std::move(*rhs)}, followed by \tcode{rhs.val->T::\~{}T()},
\tcode{swap(init, rhs.init)}, otherwise
\item (if both \tcode{*this} and \tcode{rhs} are engaged) calls
\tcode{swap(*(*this), *rhs)}.
\end{itemize}

\pnum
\throws Any exceptions that the expressions in the Effects clause throw.
...
\exceptionsafety If any exception is thrown, the values of \tcode{init} and
\tcode{rhs.init} remain unchanged. If an exception is thrown during the call to
function \tcode{swap} the state of \tcode{*val} and \tcode{*rhs.val} is
determined by the exception safety guarantee of \tcode{swap} for lvalues of
\tcode{T}. If an exception is thrown during the call to \tcode{T}'s move
constructor, the state of \tcode{*val} and \tcode{*rhs.val} is determined by
the exception safety guarantee of \tcode{T}'s move constructor.
\end{itemdescr}
...
\pnum
\remarks Unless \tcode{T} is a user-defined type with overloaded unary
\tcode{operator\&}, the first function shall be a \tcode{constexpr} function.
\end{itemdescr}
...
\pnum
\remarks The first function shall be a \tcode{constexpr} function.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\returns \tcode{*val}, if \tcode{bool(*this)}.

\pnum
\throws \tcode{bad_optional_access} if \tcode{!*this}.
...
\pnum
\requires \tcode{is_copy_constructible<T>::value} is true and
\tcode{is_convertible<U\&\&, T>::value} is true.

\pnum
\returns \tcode{bool(*this) ? **this : static_cast<T>(std::forward<U>(v))}.

\pnum
\throws Any exception thrown by the selected constructor of \tcode{T}.

\pnum
\exceptionsafety If \tcode{init == true} and an exception is thrown during the
call to \tcode{T}'s constructor, the value of \tcode{init} and \tcode{v}
remains unchanged and the state of \tcode{*val} is determined by the exception
safety guarantee of the selected constructor of \tcode{T}. Otherwise, when an
exception is thrown during the call to \tcode{T}'s constructor, the value of
\tcode{*this} remains unchanged and the state of \tcode{v} is determined by the
exception safety guarantee of the selected constructor of \tcode{T}.

\pnum
\remarks If both constructors of \tcode{T} which could be selected are
\tcode{constexpr} constructors, this function shall be a \tcode{constexpr}
function.
\end{itemdescr}
...
\pnum
\returns \tcode{bool(*this) ? std::move(**this) : static_cast<T>(std::forward<U>(v))}.
...
call to \tcode{T}'s constructor, the value of \tcode{init} and \tcode{v}
remains unchanged and the state of \tcode{*val} is determined by the exception
safety guarantee of \tcode{T}'s constructor. Otherwise, when an exception is
thrown during the call to \tcode{T}'s constructor, the value of \tcode{*this}
remains unchanged and the state of \tcode{v} is determined by the exception
safety guarantee of the selected constructor of \tcode{T}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
The struct \tcode{in_place_t} is an empty structure type used as a unique type
to disambiguate constructor and function overloading. Specifically,
\tcode{optional<T>} has a constructor with \tcode{in_place_t} as the first
argument followed by an argument pack; this indicates that \tcode{T} should be
constructed in-place (as if by a call to placement \tcode{new} expression) with
the forwarded argument pack as parameters.
\end{itemdescr}

\rSec2[optional.nullopt]{Disengaged state indicator}
...
\begin{itemdescr}
\pnum
The struct \tcode{nullopt_t} is an empty structure type used as a unique type
to indicate a disengaged state for \tcode{optional} objects. In particular,
\tcode{optional<T>} has a constructor with \tcode{nullopt_t} as single argument;
this indicates that a disengaged optional object shall be constructed.

\pnum
The type \tcode{nullopt_t} shall not have a default constructor. It shall be a
literal type. The constant \tcode{nullopt} shall be initialized with an
argument of literal type.
\end{itemdescr}

\rSec2[optional.bad_optional_access]{Class \tcode{bad_optional_access}}

\indexlibrary{\idxcode{bad_optional_access}}%
\begin{codeblock}
namespace std {
...
\pnum
The class \tcode{bad_optional_access} defines the type of objects thrown as
exceptions to report the situation where an attempt is made to access the value
of a disengaged optional object.
...
\begin{itemdescr}
\pnum
\effects Constructs an object of class \tcode{bad_optional_access}.

\pnum
\postcondition \tcode{strcmp(what(), what_arg.c_str()) == 0}.
\end{itemdescr}
...
\pnum
\postcondition \tcode{strcmp(what(), what_arg) == 0}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\requires \tcode{T} shall meet the requirements of \tcode{EqualityComparable}.

\pnum
\returns If \tcode{bool(x) != bool(y)}, \tcode{false}; otherwise if
\tcode{bool(x) == false}, \tcode{true}; otherwise \tcode{*x == *y}.

\pnum
\remarks Instantiations of this function template for which \tcode{*x == *y} is
a core constant expression, shall be \tcode{constexpr} functions.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\requires Expression \tcode{less<T>\{\}(*x, *y)} shall be well-formed.

\pnum
\returns If \tcode{(!y)}, false; otherwise, if \tcode{(!x)}, true; otherwise
\tcode{less<T>\{\}(*x, *y)}.

\pnum
\remarks Instantiations of this function template for which
\tcode{less<T>\{\}(*x, *y)} is a core constant expression, shall be
\tcode{constexpr} functions.
\end{itemdescr}
...
\begin{itemdescr}
\pnum \returns \tcode{bool(x) ? *x == v : false}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum \returns \tcode{bool(x) ? v == *x : false}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum \returns \tcode{bool(x) ? less<T>\{\}(*x, v) : true}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum \effects calls \tcode{x.swap(y)}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum \returns \tcode{optional<typename decay<T>::type>(std::forward<T>(v))}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\requires The template specialization \tcode{hash<T>} shall meet the
requirements of class template \tcode{hash}\brk{}~(\ref{unord.hash}).
The template specialization \tcode{hash<optional<T>>} shall meet the
requirements of class template \tcode{hash}. For an object \tcode{o} of type
\tcode{optional<T>}, if \tcode{bool(o) == true}, \tcode{hash<optional<T>>()(o)}
shall evaluate to the same value as \tcode{hash<T>()(*o)}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects If \tcode{get() == nullptr} there are no
effects. Otherwise \tcode{get_deleter()(get())}.
...
\begin{itemdescr}
\pnum\requires The second and third constructors shall not participate in the
...
\begin{itemdescr}
\pnum\returns  \tcode{expired() ? shared_ptr<T>() : shared_ptr<T>(*this)}.
\end{itemdescr}
...
\rSec4[util.smartptr.ownerless]{Class template \tcode{owner_less}}
...
\begin{itemdescr}
\pnum\requires The template specialization shall meet the requirements of class
...
the same value as \tcode{hash<typename UP::pointer>()(p.get())}. The specialization
\tcode{hash<typename UP::pointer>} shall be well-formed.
\end{itemdescr}
...
namespace std {
  ...
  // \ref{unord.hash}, hash function base template:
  template <class T> struct hash;
...
to have a BaseCharacteristics of \tcode{true_type} to indicate that
...
\begin{itemdescr}
\pnum
\requires \tcode{F} shall be \tcode{CopyConstructible}. \tcode{f} shall
be Callable~(\ref{func.wrap.func}) for argument types
\tcode{ArgTypes} and return type \tcode{R}. The copy
constructor and destructor of \tcode{A} shall not throw exceptions.

\pnum
\postconditions \tcode{!*this} if any of the following hold:
\begin{itemize}
\item \tcode{f} is a NULL function pointer.
\item \tcode{f} is a NULL pointer to member.
...
\begin{itemdescr}
\pnum\effects If \tcode{*this != NULL}, destroys the target of \tcode{this}.
...
\begin{itemdescr}
\pnum\requires the template specializations shall meet the requirements of class template
\tcode{hash}~(\ref{unord.hash}).
\end{itemdescr}
...
define the modification. It shall define a nested type named \tcode{type},
which shall be a synonym for the modified type.
...
\tcode{template <class T>}\br
  ...
  \tcode{is_arithmetic<T>::value} \tcode{\&\&}
  \tcode{T(-1) < T(0)}   &   \\  \rowsep
...
\tcode{template <class T>}\br
  ...
  \tcode{is_constructible<T, const T\&>::value} is \tcode{true}. &
  \tcode{T} shall be a complete type, (possibly \cv-qualified) \tcode{void},
  or an array of unknown bound. \\ \rowsep

\tcode{template <class T>}\br
  \tcode{struct is_move_constructible;} &
  \tcode{is_constructible<T, T\&\&>::value} is \tcode{true}. &
  \tcode{T} shall be a complete type, (possibly \cv-qualified) \tcode{void},
  or an array of unknown bound. \\ \rowsep
...
\tcode{template <class T>}\br
  ...
  \tcode{is_assignable<T\&, const T\&>::value} is \tcode{true}. &
  \tcode{T} shall be a complete type, (possibly \cv-qualified) \tcode{void},
  or an array of unknown bound. \\ \rowsep

\tcode{template <class T>}\br
  \tcode{struct is_move_assignable;} &
  \tcode{is_assignable<T\&, T\&\&>::value} is \tcode{true}. &
  \tcode{T} shall be a complete type, (possibly \cv-qualified) \tcode{void},
  or an array of unknown bound. \\ \rowsep
...
\tcode{template <class T>}\br
 ...
 \tcode{is_trivially_constructible<T, const T\&>::value} is \tcode{true}. &
  \tcode{T} shall be a complete type,
 (possibly cv-qualified) void, or an array of unknown
 bound.                \\ \rowsep
...
\tcode{template <class T>}\br
 ...
 \tcode{is_trivially_constructible<T, T\&\&>::value} is \tcode{true}. &
  \tcode{T} shall be a complete type,
 (possibly cv-qualified) void, or an array of unknown
 bound.                \\ \rowsep
...
\tcode{template <class T>}\br
 ...
 \tcode{is_trivially_assignable<T\&, const T\&>::value} is \tcode{true}. &
 \tcode{T} shall be a complete type,
 (possibly cv-qualified) void, or an array of unknown
 bound.                \\ \rowsep
...
\tcode{template <class T>}\br
 ...
 \tcode{is_trivially_assignable<T\&, T\&\&>::value} is \tcode{true}. &
 \tcode{T} shall be a complete type,
...
\tcode{template <class T>}\br
 ...
 \tcode{is_nothrow_constructible<T, const T\&>::value} is \tcode{true}. &
 \tcode{T} shall be a complete type,
 (possibly cv-qualified) void, or an array of unknown
 bound.                \\ \rowsep
...
\tcode{template <class T>}\br
 ...
 \tcode{is_nothrow_constructible<T, T\&\&>::value} is \tcode{true}. &
 \tcode{T} shall be a complete type,
 (possibly cv-qualified) void, or an array of unknown bound.                \\ \rowsep
...
\tcode{template <class T>}\br
 ...
 \tcode{is_nothrow_assignable<T\&, const T\&>::value} is \tcode{true}. &
 \tcode{T} shall be a complete type,
 (possibly cv-qualified) void, or an array of unknown
 bound.                \\ \rowsep
...
\tcode{template <class T>}\br
  ...
  \tcode{is_nothrow_assignable<T\&, T\&\&>::value} is \tcode{true}. &
 \tcode{T} shall be a complete type,
 (possibly cv-qualified) void, or an array of unknown
 bound.                \\ \rowsep
...
resolution unless \tcode{Rep2} is implicitly convertible to \tcode{rep} and
\begin{itemize}
...
\begin{itemdescr}
\pnum
\effects value-initializes the OuterAlloc base class and the \tcode{inner} allocator
object.
\end{itemdescr}
...
allocator_arg, inner_allocator_type()), x)}.
...
then \tcode{xprime} is
\tcode{tuple_cat(x, tuple<inner_allocator_type\&>(inner_allocator_type()))}.
...
\tcode{tuple_cat(tuple<allocator_arg_t, inner_allocator_type\&>(
allocator_arg, inner_allocator_type()), y)}.
...
then \tcode{yprime} is
\tcode{tuple_cat(y, tuple<inner_allocator_type\&>(inner_allocator_type()))}.
