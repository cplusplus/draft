...
initializations for copy and move, respectively, would meet the
...
would meet the requirements for a constexpr function. The
...
and shall meet the \oldconcept{Destructible} requirements (\tref{destructible}).
...
value-initialization of the alternative type $\tcode{T}_0$ would meet the
requirements for a constexpr function.
The expression inside \tcode{noexcept} is equivalent to
...
D>::pointer} shall
meet the \oldconcept{NullablePointer} requirements (\tref{nullablepointer}).
...
meet the \oldconcept{DefaultConstructible} requirements (\tref{defaultconstructible}),
...
meets the \oldconcept{Allocator} requirements (\tref{utilities.allocator.requirements}).
...
\begin{itemize}
\item meet the \oldconcept{Hash} requirements (\tref{hash}),
