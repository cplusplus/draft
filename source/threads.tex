...
\ref{thread.mutex}    & Mutual exclusion      & \tcode{<mutex>}               \\ \rowsep
...
should, but might not, decrease the duration of the wait when the clock is adjusted forwards.
\exitnote
...
\pnum
\throws Nothing if \tcode{Clock} satisfies the \tcode{TrivialClock}
requirements~(\ref{time.clock.req}) and operations of \tcode{Duration}
do not throw exceptions. \enternote instantiations of time point types and
clocks supplied by the implementation as specified in~\ref{time.clock} do
not throw exceptions. \exitnote
\end{itemdescr}
...
\pnum
\throws Nothing if operations of \tcode{chrono::duration<Rep, Period>} do not
throw exceptions. \enternote instantiations of duration types supplied by the
implementation as specified in~\ref{time.clock} do not throw exceptions.
\exitnote
\end{itemdescr}
...
\tcode{std::recursive_mutex}, \tcode{std::timed_mutex}, and \tcode{std::recursive_timed_mutex}.
...
\begin{itemdescr}
\pnum
\requires If \tcode{m} is of type \tcode{std::mutex} or \tcode{std::timed_mutex}, the calling
...
\pnum
\sync This operation \term{synchronizes with}~(\ref{intro.multithread}) subsequent
...
\pnum
The \defn{timed mutex types} are the standard library types \tcode{std::timed_mutex} and
\tcode{std::recursive_timed_mutex}. They shall meet the requirements set out below.
...
\begin{itemdescr}
\pnum
\precondition If the tick \tcode{period} of \tcode{rel_time} is not exactly convertible to the
native tick \tcode{period}, the \tcode{duration} shall be rounded up to the nearest native tick
\tcode{period}. If \tcode{m} is of type \tcode{std::timed_mutex}, the calling thread does not
own the mutex.
...
\begin{itemdescr}
\pnum
\requires If \tcode{m} is of type \tcode{std::timed_mutex}, the calling thread does not own the
mutex.
...
\pnum \errors
\begin{itemize}
\item \tcode{invalid_argument} --- if the \tcode{once_flag} object is no longer valid.
\end{itemize}
...
\begin{itemdescr}
\pnum\effects If any threads are blocked waiting for \tcode{*this}, unblocks one of those theads.
\end{itemdescr}
...
\item If the function exits via an exception, \tcode{lock.lock()} shall be
...
\pnum \errors
\begin{itemize}
\item equivalent error condition from \tcode{lock.lock()} or \tcode{lock.unlock()}.
\end{itemize}
\end{itemdescr}

\indexlibrary{\idxcode{condition}!\idxcode{wait}}%
\indexlibrary{\idxcode{wait}!\idxcode{condition}}%
\begin{itemdecl}
template <class Predicate>
  void wait(unique_lock<mutex>& lock, Predicate pred);
\end{itemdecl}
...
\pnum
\throws \tcode{std::system_error} when an exception is required~(\ref{thread.req.exception}).

\pnum
\errors
\begin{itemize}
\item equivalent error condition from \tcode{lock.lock()} or
\tcode{lock.unlock()}.
\end{itemize}
\end{itemdescr}
...
\item
If the function exits via an exception, \tcode{lock.lock()} shall be called prior to exiting the function scope.
\end{itemize}
...
\indexlibrary{\idxcode{condition}!\idxcode{wait_for}}%
\indexlibrary{\idxcode{wait_for}!\idxcode{condition}}%
\begin{itemdecl}
template <class Rep, class Period>
  cv_status wait_for(unique_lock<mutex>& lock,
                     const chrono::duration<Rep, Period>& rel_time);
...
\indexlibrary{\idxcode{condition}!\idxcode{wait_until}}%
\indexlibrary{\idxcode{wait_until}!\idxcode{condition}}%
\begin{itemdecl}
template <class Clock, class Duration, class Predicate>
  bool wait_until(unique_lock<mutex>& lock,
                  const chrono::time_point<Clock, Duration>& abs_time,
...
\errors
\begin{itemize}
\item \tcode{resource_unavailable_try_again} --- if any native handle type
...
\pnum
The \tcode{enum} type \tcode{launch} is an \impldef{extensions to \tcode{enum}
type \tcode{launch}} bitmask type~(\ref{bitmask.types}) with
...
\pnum
An shared state is \defn{ready} only if it holds a value or an exception ready for
retrieval.
...
\item for the first version, any exception thrown by the copy constructor of \tcode{R}, or
\item for the second version, any exception thrown by the move constructor of \tcode{R}.
\end{itemize}
...
\tcode{future} object that does not refer to an
shared state.
...
\begin{itemize}
\item
\tcode{future::get()} returns the value stored in the object's shared state.
If the type of the value is \tcode{MoveAssignable} the returned value is moved, otherwise it is
copied.
...
object}\term{empty} \tcode{shared_future} object that does not refer to an
shared state.
...
\pnum
The template function \tcode{async} provides a mechanism to launch a function potentially
...
\tcode{\textit{INVOKE}(g, xyz)} where \tcode{g} is the stored value of 
\tcode{\textit{DECAY_COPY}(std::forward<F>(f))} and \tcode{xyz} is the stored copy of
\tcode{\textit{DECAY_COPY}(std::forward<Args>(args))...}. The shared state is not
...
Once evaluation of \tcode{\textit{INVOKE}(g, xyz)} begins, the function is no longer
considered deferred.
...
block until the associated thread has completed, as if joined~(\ref{thread.thread.member});
...
\pnum
\throws \tcode{system_error} if \tcode{policy} is \tcode{launch::async} and the
...
\pnum
\sync a successful call to \tcode{operator()} synchronizes
with~(\ref{intro.multithread}) a call to any member function of a \tcode{future}
or
\tcode{shared_future} object that shares the shared state of \tcode{*this}. The
completion of the invocation
of the
stored task and the
storage of the result (whether normal or exceptional) into the shared state
synchronizes with~(\ref{intro.multithread}) the successful return from any member
function that detects that the state is set to ready.
\enternote
\tcode{operator()} synchronizes and serializes with other functions through the
shared state. \exitnote
\end{itemdescr}
