...
\pnum
The class \tcode{shared_mutex} shall satisfy all of the requirements for
...
\pnum
An object of type \tcode{lock_guard} controls the ownership of lockable objects
within a scope. A \tcode{lock_guard} object maintains ownership of lockable
objects throughout the \tcode{lock_guard} object's lifetime~(\ref{basic.life}).
The behavior of a program is undefined if the lockable objects referenced by
\tcode{pm} do not exist for the entire lifetime of the \tcode{lock_guard}
object.
When \tcode{sizeof...(MutexTypes)} is \tcode{1},
the supplied \tcode{Mutex} type
shall meet the \tcode{BasicLockable} requirements.
Otherwise, each of the mutex types
shall meet the \tcode{Lockable} requirements~(\ref{thread.req.lockable.basic}).
...
\begin{itemdescr}
\pnum
\precondition If a \tcode{MutexTypes} type is not a recursive mutex,
...
\begin{itemdescr}
\pnum
\precondition The calling thread owns all the mutexes in \tcode{m}.
...
\begin{itemdescr}
\pnum
\effects For all \tcode{i} in \tcode{[0, sizeof...(MutexTypes))},
...
\begin{itemdescr}
\pnum\postconditions \tcode{pm == u_p.pm} and \tcode{owns == u_p.owns} (where \tcode{u_p} is the state of \tcode{u} just prior to this construction),  \tcode{u.pm == nullptr} and \tcode{u.owns == false}.
\end{itemdescr}
...
condition of \tcode{operation_not_permitted} if \tcode{pm == nullptr}. \tcode{system_error}
...
condition of \tcode{operation_not_permitted} if \tcode{pm == nullptr}. \tcode{system_error} with an
...
\pnum\postconditions \tcode{pm == nullptr} and \tcode{owns == false}.
\end{itemdescr}
...
\tcode{operation_not_permitted} if \tcode{pm == nullptr}.
\tcode{system_error} with an error condition of
...
\begin{itemdescr}
\pnum
\precondition There shall be no thread blocked on \tcode{*this}. \begin{note} That is, all
...
\begin{itemdescr}
\pnum
\effects As if by \tcode{\textit{INVOKE}(f, t1, t2, ..., tN, R)},
where \tcode{f} is the
