...
\begin{itemdescr}
\pnum\returns \tcode{true} if and only if \tcode{x} and \tcode{y} represent the same
...
\pnum
A shared state is \defn{ready} if and only if it holds a value or an exception ready for
retrieval.
...
\begin{itemdescr}
\pnum
\returns \tcode{true} if and only if \tcode{*this} refers to a shared state.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\returns \tcode{true} if and only if \tcode{*this} has a shared state.
\end{itemdescr}
