...
\tcode{std::recursive_timed_mutex}, and \tcode{std::shared_mutex}. They shall
...
\begin{itemdescr}
\pnum
\precondition If \tcode{m} is of type \tcode{std::timed_mutex}, the calling thread does not
own the mutex.
...
\pnum
The standard library type \tcode{std::shared_mutex} is a
\term{shared mutex type}. Shared mutex types shall meet the requirements of
...
\pnum
\effects If the tick \tcode{period} of \tcode{rel_time} is not exactly
convertible to the native tick \tcode{period}, the \tcode{duration} shall be
rounded up to the nearest native tick \tcode{period}. Attempts to obtain
...
\pnum
The class \tcode{shared_mutex} provides a non-recursive mutex with shared
ownership semantics.

\pnum
The class \tcode{shared_mutex} shall satisfy all of the \tcode{SharedMutex}
requirements~(\ref{thread.sharedmutex.requirements}). It shall be a
...
\item a thread attempts to recursively gain any ownership of a \tcode{shared_mutex}.
...
requirements~(\ref{thread.sharedmutex.requirements}).
...
\pnum
\throws \tcode{system_error} when an exception is
required~(\ref{thread.req.exception}), timeout-related
...
\pnum\throws \tcode{system_error} when
an exception is required~(\ref{thread.req.exception}) or timeout-related
...
\pnum
\throws \tcode{system_error} when
an exception is required~(\ref{thread.req.exception}), timeout-related
exceptions~(\ref{thread.req.timing}), or any exception thrown by \tcode{pred}.
