...
\defnadj{prospective}{destructor} is introduced by a
\pnum
...
its declarator shall be a function declarator\iref{dcl.fct} of the form
\begin{ncbnf}
ptr-declarator \terminal{(} parameter-declaration-clause \terminal{)} \opt{noexcept-specifier} \opt{attribute-specifier-seq}
\end{ncbnf}
where the \grammarterm{ptr-declarator} consists solely of an
...
void f(Y& a) {
  if (a) {          // error: ambiguous between \tcode{X::operator int()} and \
code{Y::operator char()}
  }
}
\end{codeblock}
\end{example}
...
\end{example}
The initialization and destruction of static data members are described in
...
an \defn{anonymous union member} if it is a non-static data member or
an \defn{anonymous union variable} otherwise.
...
that corresponds\iref{basic.scope.scope} to a declaration of $F$,
