...
\tcode{MATCHING-SIG(F1, F2)} is \tcode{true} if and only if
...
\item
Otherwise, true if \tcode{\libconcept{derived_from}<Q, forwarding_query_t>} is \tcode{true}.
\item
...
\tcode{\exposid{COMPL-DOMAIN}(set_error_t)}, and
\tcode{\exposid{COMPL-DO\-MAIN}(set_stopped_t)} are ill-formed,
...
When \tcode{\libconcept{sender_in}<Sndr, Env>} is \exposid{false},
...
namespace std::execution {
  template<@\exposconcept{class-type}@ Promise>
    struct with_awaitable_senders {
      ...
      coroutine_handle<> continuation() const noexcept { return @\exposid{continuation}@; }
