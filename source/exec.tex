...
\pnum
\remarks
Any allocations performed by construction of \exposid{sender} or
...
\pnum
\effects
Returns an object of type \exposid{sender} containing a sender
initialized with \tcode{schedule(SCHED(*this))}.
\end{itemdescr}
...
\pnum
\effects
Equivalent to: \tcode{return lhs == SCHED(rhs);}
\end{itemdescr}
...
\pnum
\returns
\tcode{false} if type of \tcode{SCHED(lhs)} is not \tcode{Sch},
otherwise \tcode{SCHED(lhs) == rhs;}
\end{itemdescr}
...
\tcode{coroutine_handle<promise_type>::from_promise(*this)}.
\end{itemdescr}
...
operation associated with \tcode{STATE(*this)} by invoking:
...
returns \tcode{as_awaitable(std::forward <Sender>(sndr), *this)};
otherwise returns \tcode{as_awaitable(affine_on(std::forward<Sender>(sndr),
SCHED(*this)), *this)}.
\end{itemdescr}
...
\pnum
\effects
Completes the asynchronous operation associated with \tcode{STATE(*this)}
by invoking \tcode{set_stopped(std::move(RCVR(*this)))}.
\end{itemdescr}
