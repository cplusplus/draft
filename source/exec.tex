...
type\linebreak\tcode{inline_scheduler} and is potentially-throwing % avoid Overfull
...
initialized with\linebreak \tcode{schedule(SCHED(*this))}. % avoid Overfull
\end{itemdescr}
...
env_type<decltype(get_env(\linebreak declval<Rcvr>()))>} if that % avoid Overfull
...
\tcode{\exposid{stoppable-callback-for}<Fn,\linebreak stop_token_type, % avoid Overfull
Init>}.
\end{itemize}
