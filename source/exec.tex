...
\begin{itemdescr}
\pnum
\effects: Initialize \exposid{sch_} with
...
\begin{itemdescr}
\pnum
\effects Returns an object of type \exposid{sender} containing
...
\begin{itemdescr}
\pnum
\effects Equivalent to: \tcode{return lhs == SCHED(rhs);}
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\returns \tcode{false} if type of \tcode{SCHED(lhs)} is not
...
\pnum
\exposid{sender} is an exposition-only class that models
...
\pnum
\effects Let \tcode{r} be an object of a type that models
...
\begin{itemdescr}
\pnum
\effects Equivalent to \tcode{start(st)} where \tcode{st} is
...
\begin{itemdescr}
\pnum
\effects Initializes \exposid{handle} with
...
\begin{itemdescr}
\pnum
\expects \tcode{bool(\exposid{handle})} is \tcode{true}.
...
\begin{itemdescr}
\pnum
\effects Let \tcode{prom} be the object
...
\begin{itemdescr}
\pnum
\mandates The first parameter of type \tcode{allocator_arg_t} (if
any) is not the last parameter.

\pnum
\effects If \tcode{Args} contains an element of type \tcode{allocator_arg_t}
...
\begin{itemdescr}
\pnum
\returns A \tcode{task} object whose member \exposid{handle}
...
\begin{itemdescr}
\pnum
\returns An awaitable object of unspecified type \iref{expr.await}
...
\begin{itemdescr}
\pnum
\mandates \tcode{std::move(err.error)} is convertible to
...
\begin{itemdescr}
\pnum
\returns If \tcode{same_as<inline_scheduler, scheduler_type>} is
...
\begin{itemdescr}
\pnum
\effects Equivalent to: \tcode{returns
...
\begin{itemdescr}
\pnum
\effects If the signature \tcode{set_error_t(exception_ptr)} is
    not an element of \tcode{error_types}, calls \tcode{terminate()}
    \iref{except.terminate}. Otherwise, stores
    \tcode{current_exception()} into \exposid{errors}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Completes the asynchronous operation
...
\begin{itemdescr}
\pnum
If there is no parameter with type \tcode{allocator_arg_t}
...
\pnum
\effects Initializes an allocator \tcode{palloc} of type \tcode{PAlloc}
...
\begin{itemdescr}
\pnum
\expects \tcode{pointer} was returned from an invocation
    of the above overload of \tcode{operator new} with a size argument
    equal to \tcode{size}.

\pnum
\effects Deallocates the storage pointed to by \tcode{pointer}
