...
\pnum
\ifndrxref{basic.link.entity.same.module}
If an entity has multiple declarations
attached to different modules
where neither is reachable from the other
...
module;
export module M;
export int h();     // \#1, attached to \tcode{M}
\end{codeblocktu}
...
import "a.h";
import "b.h";
auto n = decltype(a)::b;        // ill-formed no diagnostic required, more than one unnamed enum
...
int main() {
  A a;      // ill-formed no diagnostic required, virtual function that is not pure but has no definition
}
\end{codeblock}
\end{example}
...
#include "a.h"
int main() {
    f<int>();   // ill-formed no diagnostic required, function template implicitly
