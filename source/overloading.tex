...
A a8{i,i};          // error: \#3 deduces to \tcode{A<int\&>}, \#1 and \#2 declare same constructor
...
void test() {
  auto ep = &X<true>::e;        // OK, placeholder type deduction succeeds,
                                // then selects \#1 as more partial-ordering-constrained
...
\rSec3[over.inc.general]{General}
...
\rSec3[over.inc.default]{Defaulted function}

\pnum
A defaulted postfix increment or decrement operator function
for a type \tcode{C} shall be a non-template function that
\begin{itemize}
\item
has a first parameter of type ``reference to \tcode{C}'' or
a first parameter of type ``reference to volatile \tcode{C}'',
where the implicit object parameter (if any)
is considered to be the first parameter,
\item
is defined as defaulted in \tcode{C} or
in a context where \tcode{C} is complete, and
\item
has a declared return type of \tcode{C}.
\end{itemize}
\begin{note}
A postfix increment or decrement operator
always has a parameter of type \tcode{int}\iref{over.inc.general}.
\end{note}
Name lookups and access checks
in the implicit definition\iref{dcl.fct.def.default} of
a defaulted postfix increment or decrement operator function
are performed from a context equivalent to its \grammarterm{function-body}.
A definition of a postfix increment or decrement operator as defaulted
that appears in a class shall be the first declaration of that function.
\begin{example}
\begin{codeblock}
struct S;
S operator++(S&, int) = default;        // error: \tcode{S} is not complete
\end{codeblock}
\end{example}
...
\begin{itemize}
\item
\tcode{C} is a class type for which overload resolution\iref{over.match},
as applied to direct-initialization of a variable of type \tcode{C}
from an lvalue of type \tcode{C}, does not result in a usable candidate,
\item
\tcode{C} has a destructor that is deleted or inaccessible
from the context of the \grammarterm{function-body}, or
\item
for an lvalue \tcode{c} of type \tcode{C},
overload resolution as applied to
\tcode{++c} for a postfix increment operator function or
\tcode{--c} for a postfix decrement operator function
does not result in a usable candidate.
\end{itemize}

\pnum
The implicit definition of
a defaulted postfix increment or decrement operator function \tcode{F}
that is not defined as deleted for a type \tcode{C} is equivalent to
\begin{codeblock}
C @\exposid{tmp}@(c);
++c;
return @\exposid{tmp}@;
...
where \exposid{tmp} is a variable defined for exposition only, and
\tcode{c} is an lvalue that denotes \tcode{*this}
if \tcode{F} is an implicit object member function, or
the first parameter of \tcode{F} otherwise.
...
enum class E1 {};
E1 operator++(E1&, int) = default;      // OK, defined as deleted; built-in \tcode{operator++}
                                        // does not exist for enumeration types
...
such a literal suffix identifier is ill-formed, no diagnostic required\ifndrdef{over.literal.reserved}.
