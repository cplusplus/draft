...
the conversion is to \tcode{X} or reference to (possibly cv-qualified) \tcode{X},
\end{itemize}
...
of one of the arguments in the call is
ambiguous~(\ref{dcl.init}).
\end{note}
...
B b;
int g(A);
int x = g(b);   // error: selects \tcode{g(A)}, but converting the argument is ambiguous (\ref{over.match.copy})
int f(C);
int f(A);
int y = f(b);   // error: ambiguous; \tcode{b} $\to$ \tcode{C} via constructor and \tcode{b} $\to$ \tcode{A} via constructor or conversion function
int f(B);                
int z = f(b);   // OK, \tcode{b} $\to$ \tcode{B} is a better conversion sequence
\end{codeblock}
\end{example}
...
double operator""_Bq(long double);                  // OK: does not use the reserved identifier \tcode{_Bq}~(\ref{lex.name})
double operator"" _Bq(long double);                 // uses the reserved identifier \tcode{_Bq}~(\ref{lex.name})
