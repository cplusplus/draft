...
\pnum
For a \grammarterm{unary-operator} \tcode{@},
an operator function \tcode{operator@} shall be
...
\tcode{\grammarterm{cast-expression} . \keyword{operator}@()};
otherwise, if a non-member function is selected,
the expression is interpreted as
\tcode{\keyword{operator}@( \grammarterm{cast-expression} )}.
\begin{note}
The operators \tcode{++} and \tcode{\dcr}\iref{expr.pre.incr}
are not \grammarterm{unary-operator}{s} and are described in~\ref{over.inc}.
\end{note}
...
\pnum
For a binary operator \tcode{@},
an operator function \tcode{\keyword{operator}@} shall be
...
If a member function is selected,
the expression is interpreted as
\tcode{$x$.\keyword{operator}@($y$)};
otherwise, if a non-member function is selected,
the expression is interpreted as
\tcode{\keyword{operator}@($x$, $y$)}.
...
\pnum
An assignment operator function shall be
a non-static member function with exactly one parameter.
For an expression \tcode{$x$ = $y$} with subexpressions $x$ and $y$,
the assignment operator function
is selected by overload resolution\iref{over.match.oper},
and the expression is interpreted as \tcode{$x$.\keyword{operator}=($y$)}.
...
\pnum
The operator function \tcode{\keyword{operator}()} shall be
a non-static member function with an arbitrary number of parameters.
...
\pnum
The operator function \tcode{\keyword{operator}[]}
...
\begin{ncsimplebnf}
\terminal{(} postfix-expression . \keyword{operator}->\terminal{(}\terminal{)} \terminal{)} \terminal{->} \opt{\keyword{template}} id-expression
\end{ncsimplebnf}
