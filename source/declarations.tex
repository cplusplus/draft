...
shall satisfy the constraints for a \grammarterm{function-body} of a constexpr function;
...
\pnum
For a
% CORE ISSUE 1358: non-template, non-defaulted
\tcode{constexpr} function or a
% CORE ISSUE 1358: non-template, non-defaulted, non-inheriting
...
every full-expression that appears in its initializer shall be a constant expression.
\enternote Each implicit conversion used in converting the initializer
expressions and each constructor call used for the initialization is
part of such a full-expression. \exitnote
\enterexample
\begin{codeblock}
struct pixel { 
  int x, y; 
