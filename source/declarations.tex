...
\pnum
\begin{note} in the case where the operand of a \grammarterm{decltype-specifier}
...
semantic constraints associated with invoking the type's destructor. \begin{example}
\begin{codeblock}
...
% FIXME: This is just a long way of writing "T shall be decltype(auto) or cv auto."
\begin{itemize}
\item
if the placeholder is \tcode{decltype(auto)},
then \tcode{T} shall be \tcode{decltype(auto)};
\item
otherwise, \tcode{T} shall be \cv{} \tcode{auto}.
\end{itemize}
