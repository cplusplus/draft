...
\pnum
For a coroutine \textit{f} that is a non-static member function, let
...
and \placeholder{P} be the class type denoted by
\placeholder{T}\tcode{::promise_type}.
Then, the coroutine behaves as if its body were:
\begin{codeblock}
{
  @\placeholder{P} \placeholder{p} \placeholder{promise-constructor-arguments}@;
  co_await @\placeholder{p}@.initial_suspend(); // initial suspend point
  try { @\placeholder{F}@ } catch(...) { @\placeholder{p}@.unhandled_exception(); }
@\placeholder{final-suspend}@:
  co_await @\placeholdernc{p}@.final-suspend(); // final suspend point
}
\end{codeblock}
where an object denoted as \placeholder{p} is the \defn{promise object} of
the coroutine and its type \placeholder{P} is the \defn{promise type}
of the coroutine,
...
and \tcode{return_value} are looked up in the scope of class \placeholder{P}.
...
to \mbox{\tcode{\placeholder{p}.get_return_object()}}. A call to a
...
type \tcode{coroutine_handle<\placeholder{P}>} associated with this instance
...
The allocation function's name is looked up in the scope of \placeholder{P}.
If this lookup fails, the allocation function's name is looked up in the
global scope.
If the lookup finds an allocation function in the scope of \placeholder{P},
...
is looked up in the scope of class \placeholder{P} by class member access
...
call to \placeholder{P}\tcode{::get_return_object_on_allocation_failure()}.
...
\tcode{std::coroutine_handle<\placeholder{P}>} associated with this coroutine
...
\pnum
The deallocation function's name is looked up in the scope of \placeholder{P}.
