...
\begin{itemize}
\item \grammarterm{asm-definition}{s},
\item \grammarterm{try-block}{s}, or
\item \grammarterm{simple-declaration}{s} that define variables of static
or thread storage duration, unless each declaration satisfies the requirements
for a \tcode{constexpr} object or reference declaration.
\end{itemize}
...
constexpr int first(int n) {
  static int value = n;  // error: initializer is not a constant expression
  return value;
}
\end{codeblock}
\exitexample
...
In definition of a \tcode{constexpr} constructor,
...
\pnum
For a \tcode{constexpr} function or a \tcode{constexpr} constructor, if no function
argument values exist such that an invocation of the function would be a core
...
public: 
  ...
  constexpr bool &is_on() { return flag; }
private: 
  bool flag; 
}; 
debug_flag flag;
constexpr bool &flag_is_on = flag.is_on(); // OK
