...
void f(D* d) {
   ...
   d->A::x();    // ambiguous: \tcode{d} is not implicitly convertible to a pointer to the naming class \tcode{A}
   d->C::A::x(); // ambiguous: same reason as line directly above
   d->C::x();    // OK: \tcode{x}'s direct parent \tcode{A} is an unambiguous accessible base of the naming class \tcode{C}
   d->B::x();    // OK: \tcode{x}'s direct parent \tcode{A} is an unambiguous accessible base of the naming class \tcode{B}
}
\end{codeblock}
\exitnote
