...
\item \grammarterm{declaration-statement}{s} other than
\grammarterm{asm-definition}{s} and \grammarterm{simple-declaration}{s} which
do not contain the \tcode{constexpr} specifier and define variables of static
or thread storage duration,
\item \grammarterm{expression-statement}{s},
\item \grammarterm{compound-statement}{s},
\enternote
These restrictions apply recursively to the \grammarterm{statement}{s} within
the \grammarterm{compound-statement}.
\exitnote
\item \tcode{if} statements,
\item and at least one return statement.
\end{itemize}
...
constexpr int g(int x, int n) {
  int r = 1;
  while (--n > 0) r *= x;       // error: \tcode{while} statement not allowed
  return r;
}
\end{codeblock}
\exitexample
...
the \grammarterm{compound-statement}.
\exitnote
\item \tcode{if} statements, and
\item \tcode{return} statements
\end{itemize}
