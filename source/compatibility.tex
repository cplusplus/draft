...
particular, destructors that throw exceptions will call \tcode{terminate()}
(without calling \tcode{unexpected()}) if their exception specification is
\tcode{noexcept} or \tcode{noexcept(true)}. For a throwing virtual destructor
of a derived class, \tcode{terminate()} can be avoided only if the base class
