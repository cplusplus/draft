...
void fly(volatile struct pterosaur* pteranodon);        // OK, pointer isn't volatile, data it points to is
\end{codeblock}
\end{example}
...
void park(linhenykus alvarezsauroid) {
  volatile auto [what_is_this] = alvarezsauroid;        // deprecated: binding copies the \tcode{forelimb}s?
  // ...
}
\end{codeblock}
\end{example}
