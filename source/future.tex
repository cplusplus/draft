...
++velociraptor;                 // deprecated: is this a single memory operation, or two
\end{codeblock}
\end{example}
...
tail = brachiosaur = neck;          // deprecated: does this access \tcode{brachiosaur} once or twice?
brachiosaur += neck;                // deprecated: does this access \tcode{brachiosaur} once or twice?
...
\begin{example}
\begin{codeblock}
volatile struct amber jurassic();  // deprecated: a volatile return type has no meaning
...
void fly(volatile struct pterosaur* pteranodon);   // OK, the pointer isn't volatile, the data it points to is
\end{codeblock}
\end{example}
...
void park(linhenykus alvarezsauroid) {
  auto [what_is_this] = alvarezsauroid;    // deprecated: does the binding copy the \tcode{forelimb}s?
  // ...
}
\end{codeblock}
\end{example}
