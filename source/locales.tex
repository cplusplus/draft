...
facet templates identified as members of a category,
and for those specializations of the forms shown in \tref{locale.spec}, where:
\begin{itemize}
\item
\tcode{InputIterator} denotes any type that meets
the \oldconcept{InputIterator} requirements.
\item
\tcode{OutputIterator} denotes any type that meets
the \oldconcept{OutputIterator} requirements.
\item
\exposid{C} denotes any type from an implementation-defined set of character types,
including at least \tcode{char} and \tcode{wchar_t}, that meet the requirements for
a character on which any of the iostream components can be instantiated, and
\item
\exposid{International} denotes any value of type \tcode{bool}.
\end{itemize}
...
\tcode{charT}
value or values.\footnote{The parameter \tcode{c} of
\tcode{do_widen}
