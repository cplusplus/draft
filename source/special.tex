...
\item otherwise, if the entity is a variant member~(\ref{class.union}), no initialization is performed;
...
is
\term{implicitly defined}
if it is odr-used~(\ref{basic.def.odr}) to initialize an object of its class type from a copy of an
...
is
trivial
if it is not user-provided, its declared paraemeter type is the same
...
\item the \grammarterm{exception-specification}~(\ref{except.spec}),
...
constructor template~(\ref{temp.over.link}) in the class where the using-declaration
appears. \enternote Default arguments are not inherited. \exitnote
