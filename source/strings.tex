...
for \tcode{pos_type} in~\ref{iostreams.limits.pos} and \ref{iostream.forward}.
...
public:
  ...
  constexpr int compare(size_type pos1, size_type n1, const value_type* s,
                        size_type n2) const;
...
\begin{libefftabvaluenarrow}{\tcode{basic_string_view(const value_type*)} effects}{tab:string.view.ctr.2}
\tcode{data_} & \tcode{str} \\
...
\begin{libefftabvaluenarrow}{\tcode{basic_string_view(const value_type*, size_type)} effects}{tab:string.view.ctr.3}
\tcode{data_} & \tcode{str} \\
\tcode{size_} & \tcode{len} \\
\end{libefftabvaluenarrow}
\end{itemdescr}
...
of a constant random access iterator~(\ref{random.access.iterators}) and
of a contiguous iterator~(\ref{iterator.requirements.general})
...
\begin{codeblock}
constexpr @\placeholder{return-type}@ @\placeholder{F}@(const value_type* s, size_type pos);
\end{codeblock}
...
Each member function of the form
\begin{codeblock}
constexpr @\placeholder{return-type}@ @\placeholder{F}@(const value_type* s, size_type pos, size_type n);
\end{codeblock}
...
Each member function of the form
\begin{codeblock}
constexpr @\placeholder{return-type}@ @\placeholder{F}@(value_type c, size_type pos);
\end{codeblock}
...
the corresponding string object (\ref{basic.string.hash}).
\end{note}
\end{itemdescr}
