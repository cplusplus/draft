...
\pnum
\throws The second form throws nothing if the allocator's
move constructor throws nothing.
...
unless the copy constructor for \tcode{Allocator} throws.
\end{itemdescr}
...
\pnum
\returns \tcode{*(begin() + pos)} if \tcode{pos < size()}, otherwise a reference to an object
of type \tcode{T} with value \tcode{charT()}; the referenced value shall not be modified.
...
\begin{itemdescr}
\pnum
\effects Calls \tcode{append(il.first(), il.size())}.
...
\pnum
\effects Calls \tcode{insert(pos, s, traits::length(s))}.
