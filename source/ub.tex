...
int main() {
  ...
  while (g.move_next())
    sum += g.current_value();
#if 0
  // undefined behavior to call \tcode{move_next()}, because \tcode{coro.resume()} is
  // undefined behavior after \tcode{final_suspend} returns, even when it returns \tcode{suspend_always}
  g.move_next();
#endif
  return sum;
}
